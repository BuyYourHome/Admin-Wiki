...
- Defined mode: Health Check owns one mutex-protected Windows workflow-health supervisor on `WESSTUDIO`. The supervisor runs every 10 minutes from `Project Rooms\Email Monitor\config\workflow-health-registry.json`, preserves separate workflow state, isolates malformed configurations, and emits warning, critical, and recovery notifications only on transitions. After Email Monitor moved to `OFFICEASSIST` on 2026-08-22, its WesStudio supervisor enrollment was disabled to prevent false stale alerts; lifecycle configuration now targets OfficeAssist. Invoice Entry remains enrolled on WesStudio unchanged. Email Monitor liveness alerts remain pending a verified multi-machine or dedicated OfficeAssist supervisor. Options, Status, Enable, Disable, Configure, Test, and TestAlert remain the control surface.
