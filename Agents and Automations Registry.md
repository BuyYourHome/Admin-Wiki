...
- Release `0.4.4` is ready for controlled in-place deployment. It retains the hidden launcher and closed-history optimization, permits a bounded 180-second tick for measured remote-SMB latency, and adds an audited owner-bound quarantine for hash-invalid structurally terminal records. Worker identity, ownership, state, journal, schedule, pins, and exactly-once submission behavior remain unchanged.
