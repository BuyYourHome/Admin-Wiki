...
| PR Messaging Dispatcher | Machine-local deterministic worker plus dedicated dispatcher task per execution computer | Release 0.4.0 live and validated on WES-VIDEOEDITOR for Quickbooks; OFFICEASSIST and WESSTUDIO remain on their legacy fallback configuration until each machine passes its staged cutover | Every 60 seconds, 24/7; empty polls use no model tokens; legacy model heartbeats are fallback-only | `Project Room Messaging Rule.md`; `Project Rooms\PR Messaging Dispatcher\README.md`; `skills\pr-messaging-dispatcher\SKILL.md` |
...
| Email Monitor | Wiki-managed skill plus heartbeat automation, project room, direct Email Delivery endpoint, Organize mode, compact current state, and seven-day Teams rolling log | WESSTUDIO paused; OFFICEASSIST active and messaging-ready | Heartbeat runs every day from 7:45 AM through 11:00 PM Eastern; embedded dispatcher remains temporary fallback until the OFFICEASSIST deterministic worker passes cutover; direct authorized delivery handoffs remain subject to destination readiness | `skills\email-monitor\SKILL.md`; `Project Rooms\Email Monitor\README.md`; `Project Rooms\Email Monitor\config\email-monitor-log.json`; `Project Rooms\Email Monitor\working\migrations\2026-09-04-wesstudio-to-officeassist\handoff.md` |
...
- WESSTUDIO task `01a06337-1b59-7dc2-9586-6660eb7b5da7` remains local transport only, not a substitute implementation owner. OFFICEASSIST dedicated task `01a09d84-a309-7591-a790-e770fcb53dee` (`PR Messaging Dispatcher - OFFICEASSIST`) has no Codex heartbeat or recurring automation; its embedded Email Monitor stage remains fallback-only until staged validation and live promotion complete.
- Release `0.4.0` is live on WES-VIDEOEDITOR: one minute, 24/7, no model on empty polls, machine-scoped exclusive ownership, atomic claim, and a durable per-attempt journal. Synthetic `prmsg-wve-low-token-worker-validation-20260913-001` completed with one delivered attempt and no business action before live promotion at `2026-09-14T01:04:33.2988501Z`. Two subsequent empty live ticks made zero claims, submissions, model requests, or notifications. OFFICEASSIST and WESSTUDIO follow through separate staged validations.
