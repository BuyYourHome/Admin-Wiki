...
| Invoice Entry | Wiki-managed skill plus project room, standalone backup cron monitor, shared Windows health-supervisor enrollment, and dedicated task | Active and dispatchable on `OFFICEASSIST`; messaging readiness verified 2026-09-10 | Direct handoff is primary; backup monitor runs at noon and 4:00 PM without targeting the operational task; shared health supervisor performs a daily substantive review | `skills\invoice-entry\SKILL.md`; `Project Rooms\Invoice Entry\README.md`; app automation id `invoice-entry-to-projects-backup-heartbeat`; workflow-health id `invoice-entry` |
...
- Release `0.4.5` is ready for controlled in-place deployment. It retains the hidden launcher, closed-history optimization, 180-second tick, and audited integrity quarantine. It raises only the bounded canonical-manager limits to 60 seconds for list calls and 120 seconds for atomic mutation calls, always capped by the remaining tick budget. Worker identity, ownership, state, journal, schedule, pins, and exactly-once submission behavior remain unchanged.
...
- Schedule: noon and 4:00 PM Eastern as backup.
