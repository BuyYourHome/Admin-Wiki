...
- Release `0.4.0` is live on WES-VIDEOEDITOR and OFFICEASSIST: one minute, 24/7, no model on empty polls, machine-scoped exclusive ownership, atomic claim, and a durable per-attempt journal. WES-VIDEOEDITOR synthetic `prmsg-wve-low-token-worker-validation-20260913-001` and OFFICEASSIST synthetic `prmsg-officeassist-low-token-worker-docscan-validation-20260914-001` each completed with one delivered attempt and no business action before live promotion. Both machines passed two subsequent empty live ticks with zero claims, submissions, model requests, or notifications. WESSTUDIO follows through its separate staged validation.
