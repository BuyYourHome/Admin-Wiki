...
- Activation state on 2026-07-15: the saved app automation is `PAUSED`. Two app-level update attempts did not return or change the saved automation. The charter is installed in the governing skill and project room, but the heartbeat must be reactivated through a functioning Codex automation control path before scheduled runs resume.
