...
| PR Messaging Dispatcher | Machine-local deterministic worker plus dedicated dispatcher task per execution computer | Release 0.4.0 live and validated on WES-VIDEOEDITOR, OFFICEASSIST, and WESSTUDIO; WESSTUDIO Create PR destination has one preserved exhausted-ambiguity hold | Every 60 seconds, 24/7; empty polls use no model tokens; legacy model heartbeats are fallback-only | `Project Room Messaging Rule.md`; `Project Rooms\PR Messaging Dispatcher\README.md`; `skills\pr-messaging-dispatcher\SKILL.md` |
...
- Release `0.4.2` is ready for controlled in-place deployment. It replaces direct scheduled `powershell.exe` execution with a hash-pinned `wscript.exe` hidden launcher so recurring ticks create no console or focus interruption. Worker identity, ownership, state, journal, schedule, pins, and transport behavior remain unchanged.
