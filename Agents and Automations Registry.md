...
- Current queued test and remaining WES-VIDEOEDITOR pull step: [[Project Rooms/Create PR/outputs/WES-VIDEOEDITOR Dispatcher Validation Queue 2026-09-05]]. Wes's linked decision `prmsg-jean-wve-synthetic-hours-exception-20260905-003` permits the existing WVE heartbeat every five minutes continuously without expiry until Wes closes hours, only with a persistent exact synthetic filter and no production/promotion. Prior hours/expiry/restoration gates are historical. No remote override is installed or verified from WESSTUDIO; see [[Project Rooms/Create PR/outputs/WES-VIDEOEDITOR One-Hour Synthetic Exception Setup]].
