...
| Email Monitor | Wiki-managed skill plus heartbeat automation, project room, direct Email Delivery endpoint, Organize mode, compact current state, and seven-day Teams rolling log | WESSTUDIO paused; OFFICEASSIST active and messaging-ready | Heartbeat runs every day from 7:45 AM through 11:00 PM Eastern; Project Room dispatch is handled by the OFFICEASSIST deterministic worker; direct authorized delivery handoffs remain subject to destination readiness | `skills\email-monitor\SKILL.md`; `Project Rooms\Email Monitor\README.md`; `Project Rooms\Email Monitor\config\email-monitor-log.json`; `Project Rooms\Email Monitor\working\migrations\2026-09-04-wesstudio-to-officeassist\handoff.md` |
...
| Bathroom Fixtures | Wiki-managed skill plus project room plus dedicated task | Active/discovery | On demand | `skills\bathroom-fixtures\SKILL.md`; `Project Rooms\Bathroom Fixtures\README.md`; task id `01a0432b-d780-7b01-aed3-e0af40daa663` |
...
| Entity Relationship | Wiki-managed skill plus project room plus dedicated task | Active/planning; messaging registration pending | On demand | `skills\entity-relationship\SKILL.md`; `Project Rooms\Entity Relationship\README.md`; task id `01a09295-d979-7e00-ab98-9390d005a1bf` |
...
| Invoice Entry | Wiki-managed skill plus project room, dedicated task, and deterministic message delivery; legacy WESSTUDIO backup cron deleted | Active and dispatchable on `OFFICEASSIST`; messaging readiness verified 2026-09-10 | OFFICEASSIST worker delivers durable handoffs; obsolete WESSTUDIO backup deleted 2026-09-18. OFFICEASSIST substantive health/unqueued-packet coverage remains unverified | `skills\invoice-entry\SKILL.md`; `Project Rooms\Invoice Entry\README.md`; former automation id `invoice-entry-to-projects-backup-heartbeat`; workflow-health id `invoice-entry` |
...
- WESSTUDIO task `01a06337-1b59-7dc2-9586-6660eb7b5da7` remains local transport only, not a substitute implementation owner. Its deterministic worker is live for Create PR and Bathroom Fixtures; the model dispatcher heartbeat remains paused. OFFICEASSIST dedicated task `01a09d84-a309-7591-a790-e770fcb53dee` (`PR Messaging Dispatcher - OFFICEASSIST`) has no Codex heartbeat or recurring dispatcher automation; its deterministic worker is live and Email Monitor's embedded dispatcher stage is removed.
- Release `0.4.1` is live on all three computers: one minute, 24/7, no model on empty polls, machine-scoped exclusive ownership, atomic claim, a durable per-attempt journal, and guarded ambiguity handling. WES-VIDEOEDITOR synthetic `prmsg-wve-low-token-worker-validation-20260913-001`, OFFICEASSIST synthetic `prmsg-officeassist-low-token-worker-docscan-validation-20260914-001`, and WESSTUDIO replacement synthetic `prmsg-officeassist-wesstudio-low-token-worker-validation-20260914-002` each completed with one delivered attempt and no business action before live promotion. Every machine passed two subsequent Live ticks with zero claims, submissions, model requests, or notifications.
- Release `0.4.6` is ready for controlled in-place deployment. It retains the hidden launcher, closed-history optimization, 180-second tick, 60/120-second manager limits, and audited integrity quarantine. A guarded upgrade now reviews and pins the current normal-user Codex CLI path/hash after an application update replaces its versioned directory; ordinary ticks still fail closed rather than auto-trusting a new executable. Worker ownership, state, journal, schedule, pins, and exactly-once submission behavior remain unchanged.
- WESSTUDIO's first validation `prmsg-officeassist-wesstudio-low-token-worker-validation-20260914-001` remains historically `Delivery Ambiguous` with no receipt after a CLI timeout. Release `0.4.1` recorded an identity-bound administrative transport closure that releases the transport slot without claiming delivery or business completion; it is preserved without retry.
...
- Historical September 5 heartbeat validation material is preserved under [[Project Rooms/Create PR/outputs/WES-VIDEOEDITOR Dispatcher Validation Queue 2026-09-05]] and [[Project Rooms/Create PR/outputs/WES-VIDEOEDITOR One-Hour Synthetic Exception Setup]]. It does not override the release `0.4.0` worker cutover contract.
...
- If an instruction email has a subject containing `gracious millionaire`, or otherwise clearly belongs to the Gracious Millionaire book/project-room workflow, route it to `Project Rooms\Gracious Millionaire\` as book source material from the OfficeAssist monitor.
- Do not routinely preserve routed Gracious Millionaire emails or attachments as Git-tracked files. Preserve Outlook message id or web link, available sender, recipient, timestamp, subject, short summary, and any external Teams source path in applicable ledgers.
- Update `Project Rooms\Gracious Millionaire\working\officeassist-intake-log.md` and the durable source inventory with references and external paths when required by the project-room rules.
- Send a direct follow-up message to the existing Gracious Millionaire project-room thread with the Outlook reference, Teams path if any, and a short summary. Direct message handoff is the primary trigger; the project-room heartbeat is only a backup processor for external source references or files already routed to the project room.
...
- Do not routinely preserve routed Brynda Suit emails or attachments as Git-tracked files. Preserve Outlook message id or web link, available sender, recipient, timestamp, subject, short summary, and any external Teams source path in applicable ledgers.
- Update `Project Rooms\Brynda Suit\working\source-inventory.md` with references and external paths when the routed email becomes part of the durable source set.
- Send a direct follow-up message to the existing Brynda Suit task with the Outlook reference, Teams path if any, a short summary, and the instruction to wake up and respond to the email.
...
- Do not routinely preserve routed REI BlackBook website emails or attachments as Git-tracked files. Preserve Outlook message id or web link, available sender, recipient, timestamp, subject, short summary, and any external Teams source path in applicable ledgers.
- Update `Project Rooms\REI BlackBook\working\source-inventory.md` with references and external paths when the routed email becomes part of the durable source set.
- Send a direct follow-up message to the existing REI Blackbook project-room thread with the Outlook reference, Teams path if any, a short summary, and the instruction to process the website request.
...
- Run Organize after every Sent Items-verified delivery where Wes is a To or CC recipient, filing messages directly inside Wes's `Inbox/Jean Wright` into the documented Email Monitor subfolders.
...
- Defined mode: Health Check owns independent mutex-protected Windows supervisors on assigned machines. WesStudio runs `workflow-health-registry.json` with Email Monitor and Invoice Entry enabled. OfficeAssist keeps Email Monitor disabled in `officeassist-workflow-health-registry.json`; Doc Scan remains unchanged pending migration evidence review. Both use the same canonical evaluator while keeping machine-local tasks, state, diagnostics, and mutexes. The control surface selects the current machine registry by default. Options, Status, Enable, Disable, Configure, Test, and TestAlert remain available.
- Defined mode: Task Health reviews Email Monitor's operational task context and durable-state readiness. It reports measured signals and stops for Wes approval; approved rollover verifies one replacement, retargets the existing heartbeat and specifically authorized task-ID dependencies, and archives the predecessor only after delivery and routing safety checks pass.
...
- Run Git outside the Codex sandbox at logon and daily at 5:30 AM Eastern; apply only clean fast-forward pulls.
...
- Windows task: `BuyYourHome-SyncGithub`, non-administrator limited run level, at logon and daily at 5:30 AM Eastern.
...
- `C:\Codex\Wiki Files\Project Rooms\Facebook Engagement\README.md`
...
- Marketplace activity and its seller-response heartbeat remain paused until Wes explicitly resumes Marketplace.
- While paused, do not search listings, evaluate new opportunities, message sellers, monitor conversations, negotiate, or send Marketplace notifications.
...
- Messaging status: `Ready and dispatchable` on `OFFICEASSIST`. Canonical task `01a03956-fa4f-77c1-9ab7-f709e5f1174e`; lifecycle completed `2026-09-10T15:51:22.7830079Z` with one notification, one attempt, dispatcher task `01a03956-fe55-7f62-9c0a-17c18f763320`, automation `officeassist-morning-email-summary-and-instruction-monitor`, and `manual_intervention: false`. Canonical manifest: `config\pr-messaging-manifests\invoice-entry.json`.

Automation:

- Standalone cron id: `invoice-entry-to-projects-backup-heartbeat`; display name: `Invoice Entry Backup Monitor`.
- Status: **DELETED on WESSTUDIO as of 2026-09-18**, explicitly authorized by Wes after pausing. App deletion confirmed; the three redundant execution tasks were previously archived. Do not recreate the obsolete monitor without new authorization.
- Historical schedule: noon and 4:00 PM Eastern as backup; no longer scheduled.
- Current transport coverage: the OFFICEASSIST deterministic worker delivers durable messages to current Invoice Entry task `01a03956-fa4f-77c1-9ab7-f709e5f1174e`. Transport delivery is not a substitute for finding unqueued packets or substantive workflow health review. The canonical OFFICEASSIST health registry currently lists Email Monitor and Doc Scan only; Invoice Entry coverage must be verified locally before declaring those responsibilities migrated. No replacement automation was created.
- Historical app automation id: `invoice-entry-to-projects-backup-heartbeat` (deleted).
...
- Primary routed-email trigger: Email Monitor creates an authoritative central Project Room message, then sends the dedicated Invoice Entry task a best-effort wake-up message.
- Backup scope: inspect the Invoice Entry project room for new or changed structured invoice/receipt packets and inspect unresolved central Project Room messaging records addressed to Invoice Entry. Deduplicate by message id, dispatch id, and payload hash. The monitor may write `StartAttempt` and send one same-id wake-up after confirming the operational task is idle, but it must not accept or process work on behalf of Invoice Entry. Uncertain notification is `Delivery Ambiguous`; never invent acceptance, replace the record, or change the immutable payload. Do not scan inboxes, inspect raw scan folders, copy files into Teams, approve or pay invoices, contact vendors, redesign workbook templates, or create new chats.
- Execution lifecycle: the scheduler owns cron execution-task completion and visibility. The monitor does not inspect terminal metadata or prior automation tasks and does not archive execution tasks. On 2026-08-22, two stale executions from August 20 and August 21 were stopped and archived after both were found stalled in terminal-inspection calls; the self-inspection, predecessor cleanup, and self-archival prompt logic was removed.
...
- Run authorized QuickBooks workflows through explicitly named modes.
- In `Invoice` mode, receive validated, authorized, structured vendor-bill handoffs from the registered Invoice Entry task.
- Use the authorized authenticated QuickBooks execution method to create exactly one matching bill.
- Protect against duplicates, read the created record back, log the outcome, and return the verified result to Invoice Entry.

Defined in:

- `C:\Codex\Wiki Files\skills\quickbooks\SKILL.md`
- `C:\Codex\Wiki Files\Project Rooms\Quickbooks\README.md`
- `C:\Codex\Wiki Files\config\pr-messaging-manifests\quickbooks.json`
