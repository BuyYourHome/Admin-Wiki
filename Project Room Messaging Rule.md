...
- A `Delivery Ambiguous` record may receive a bounded same-ID retry when it has no accepted receipt or final state, all prior attempts are complete, its attempt count remains below the immutable maximum, and the destination currently passes its manifest gate. A destination becoming dispatchable after an earlier failed or ambiguous attempt is a new actionable condition. Reconcile and retry the existing record; never create a replacement.
