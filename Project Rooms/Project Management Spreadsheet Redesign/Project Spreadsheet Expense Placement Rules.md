...
- For `Profit` formulas that need the five-year subject-to payoff balance in option 3 / Slow Flip, look up `tblSubjectToLoan[Balance]` by `tblScheduleIndex[Period]=60`, not by the `Year` index. The Year helper can represent presentation groupings and may not be the correct key for a specific 60-month balance.
