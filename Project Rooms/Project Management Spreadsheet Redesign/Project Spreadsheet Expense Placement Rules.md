...
- For Tensity-style Amortization term inputs, treat `Contract Date` (`J12`), first-rate years (`O12`), and total contract years (`O13`) as the user-facing input fields. Calculate first-rate months in `P12`, Rate Change Date in `S12`, total contract months in `P13`, and End Date in `S13`; do not turn `S12` or `S13` into manual inputs unless Wes explicitly changes the design. The table formulas may use the calculated dates for rate switching and cutoff logic, but the input surface should remain the contract date plus year counts. Verify the rate transition row, final payoff row, summary block, and no-error status after upload.
