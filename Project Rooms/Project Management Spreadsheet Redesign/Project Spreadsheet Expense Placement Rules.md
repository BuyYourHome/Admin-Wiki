...
- When rolling out one worksheet across multiple project workbooks, treat macro-enabled `.xlsm` files that hang during Excel sheet-copy automation as a review risk, not as a normal silent fallback. If a non-Excel workbook-structure fallback is used to finish a sheet copy, preserve a rollback copy, verify ZIP integrity, sheet existence, named ranges, Tensity/source data removal, and absence of workbook-style external links, and report any material file-size change to Wes before proceeding to the next worksheet.
