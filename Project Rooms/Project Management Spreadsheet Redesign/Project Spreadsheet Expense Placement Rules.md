...
- Before uploading any workbook after a worksheet copy, replacement, or rollback rebuild, compare the full before-and-after worksheet list against the rollback/source workbook. The only allowed sheet-list changes are the exact intended ones, such as adding a preserved `Trade Properties - Old 0703` sheet and replacing the active `Trade Properties` sheet. If any unrelated sheet is missing, renamed, hidden unexpectedly, or displaced, stop and restore from rollback. In the July 2026 Trade Properties repair pass, this missing guard would have caught `.xlsx` workbooks where `Amortization` or `Profit` was accidentally removed even though the file opened cleanly and the copied Trade sheet existed.
