...
- After copying a worksheet from a source workbook into target project workbooks, scan both workbook-level and sheet-scoped defined names for external references to the source workbook before upload. Excel may also retain cached workbook links after bad defined names are deleted, so verify with Excel's workbook-link list and break unintended project-workbook links before uploading the cleaned workbook.
