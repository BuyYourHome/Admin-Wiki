...
- After Wes manually moves an Amortization Summary block, locate it by the visible title and labels before changing formulas; do not assume the previous address is still authoritative. In the Tensity table-formula pass, the active Summary block was at `Y2:AB10`, so the formula work had to preserve that moved block and update its value column in place. Convert any remaining whole-column Summary lookups, such as month-12 cash-flow lookups, to the corresponding table columns during the same pass, then verify the visible summary values, payoff row, and no-error status in both the working copy and the active synced workbook.
- For Tensity-style Amortization date inputs, treat `Contract Date`, `Rate Change Date`, and `End Date` as the user-facing source fields. Derive first-rate months/years and total contract months/years from those dates, rather than letting the date fields be inferred from old month inputs. If converting an existing workbook, preserve the current calculated date values as fixed input values before changing the month formulas, then verify the rate transition row, final payoff row, summary block, and no-error status after upload.
- In the Teams/SharePoint connector, active project-management workbooks may live directly under the `Property` drive root, while each named property folder contains supporting subfolders such as `Buying`, `Owning`, and `Renting`. Do not assume the project workbook is inside `Owning`; browse the connector root/folder structure and fetch the exact workbook item before using any local working copy.
