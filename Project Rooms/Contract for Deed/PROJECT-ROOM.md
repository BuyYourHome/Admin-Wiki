...
Apply the same active-plus-Archive versioning rule to `Contract Package\Spanish Package\`: the active Spanish Package folder should contain only the current unversioned Spanish/bilingual deliverable using the street-only property prefix, and prior Spanish/bilingual drafts should be moved into `Spanish Package\Archive` with version prefixes preserved or added when archiving an unversioned prior active file.
