...
2. Run the helper exactly once through a process-scoped execution-policy bypass. On `WES-VIDEOEDITOR`, the exact command is:
   ...
   Normal scheduled runs omit `-MessageId` and retain oldest-first selection. If Wes explicitly scopes the current run to one immutable record, append `-MessageId "<exact-message-id>"`. The helper must filter before candidate selection and fail closed if that exact record is missing or ineligible. That filter expires with the current run and must not suppress later normal scheduled runs.
