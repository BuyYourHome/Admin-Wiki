...
| 2026-09-01 | WES-VIDEOEDITOR | Helper invocation hardened | Host execution policy can block direct `.ps1` invocation. Heartbeat now requires a process-scoped `powershell.exe -ExecutionPolicy Bypass -File` call with the exact dispatcher task id. |
| 2026-09-02 | WES-VIDEOEDITOR | Pre-execution failure recovery and observability hardened | A scheduled turn generated malformed JavaScript before PowerShell started, leaving an eligible record untouched. Added one safe identical wrapper retry, structured candidate/skip diagnostics, machine-local dispatcher health, canonical-record wake-up wording, and a 120-second receipt reconciliation window. Isolated mock-manager claim and health validation passed without production queue access. |
| 2026-09-02 | WES-VIDEOEDITOR | Expired diagnostic-only turn incorrectly suppressed scheduled claim | Canonical heartbeat and messaging rules now state that one-turn diagnostic, read-only, and no-claim restrictions expire before later scheduler invocations; only a persistent Wes pause may suppress an active dispatcher. |
