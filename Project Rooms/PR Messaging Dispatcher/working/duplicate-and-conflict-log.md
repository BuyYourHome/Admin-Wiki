...
| September 7 direct real-canary authorization versus prior preparation-only gate | Authorizes exactly one fresh one-shot worker synthetic, not general real mode or deployment. The existing dispatchable Quickbooks manifest is required without altering its historical readiness validation. The exact canary is reserved from legacy StartAttempt; general transport and production rules remain unchanged. |
