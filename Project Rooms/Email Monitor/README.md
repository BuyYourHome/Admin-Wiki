...
For both mailboxes, the Time Card hierarchy has first classification priority. Keep source reports, correction threads, factual confirmations, approval requests, and daily, open-period, or refreshed drafts in the parent `Time Cards` folder. Move a message containing or transmitting the exact final Wes-approved Tim Fleming invoice to `Time Cards\Tim`; move the equivalent final Josh Kennedy or Josh Kennedy LLC invoice to `Time Cards\Josh`. A factual-accuracy confirmation or Jenny review does not establish Wes's final approval. Create either subfolder when missing.

Each activated mailbox is processed at its own `Inbox/Jean Wright` root. Organize also checks messages directly inside the parent `Time Cards` folder so qualifying approved-invoice backlog can be moved into the correct Tim or Josh subfolder; it does not revisit messages already inside those subfolders or reclassify other child-folder messages. Message state and content are preserved, and folder-access or move failures are reported. The canonical skill contains the full separate Wes and Jenny folder sets and precedence rules.
