...
| Authenticated browser access | blocked on `WES-VIDEOEDITOR` as of `2026-09-01T20:48Z` | Chrome redirected invoice `1277608` processing to Intuit sign-in with an unauthenticated session. Wes must sign in to QuickBooks in Chrome before exact company, vendor, duplicate, mapping, staging, or save gates can run. |
