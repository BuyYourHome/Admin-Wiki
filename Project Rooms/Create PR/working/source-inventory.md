...
| Wes instruction for Min PR Set mode | User instruction | authoritative | Defines a Create PR mode for setting up the minimum matching Codex chat / Project Room combinations on a prepared target computer. |
| Wes instruction to add Sync GetHub to the Min PR Set | User instruction | authoritative | Adds the Sync GetHub task to the default set and requires each target computer's daily 5:30 AM automation and first safe run to be verified or recorded as pending. |
| Corrected Create PR messaging-readiness handoff `prmsg-create-pr-messaging-readiness-gate-20260829-1715-correction-001` | Durable delegated instruction | authoritative | Requires exact task metadata, a canonical destination manifest, exact execution-machine registration, host access, one-notification synthetic lifecycle validation, a mandatory dispatchability gate, and a read-only audit of existing active PR readiness. |
| Wes instruction on 2026-09-01 | User instruction | authoritative | Requires one machine-local dispatcher capability per execution computer and an unattended cross-machine lifecycle before a remote PR is dispatchable; manual pasting is not readiness evidence. |
