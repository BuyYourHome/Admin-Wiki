...
5. Verify the target computer has one dedicated `PR Messaging Dispatcher - <COMPUTERNAME>` task and one validated release `0.4.0` deterministic worker scheduled every 60 seconds, 24/7. Record it as pending when task creation, worker staging, exclusive ownership, or unattended validation is incomplete. A legacy heartbeat or Email Monitor dispatcher stage remains fallback-only until cutover and must not overlap the worker afterward.
