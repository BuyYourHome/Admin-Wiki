...
6. If the execution machine differs from the source machine, verify that the destination computer has one dedicated machine-local PR Messaging Dispatcher task with its own task id and a validated deterministic worker. Never reuse Email Monitor, Doc Scan, Invoice Entry, or another operational destination task as the dispatcher identity. A legacy heartbeat or OfficeAssist Email Monitor dispatcher stage is temporary fallback evidence only, not the target installation.
   - The dispatcher's stored automation prompt must be a short pointer requiring every run to reread the current canonical heartbeat prompt and messaging rule. Do not copy the full dispatcher policy into the automation prompt.
...
5. Verify the target computer has one dedicated `PR Messaging Dispatcher - <COMPUTERNAME>` task with its own task id and one validated release `0.4.0` deterministic worker scheduled every 60 seconds, 24/7. Never reuse an operational destination task as the dispatcher identity. Record it as pending when task creation, worker staging, exclusive ownership, or unattended validation is incomplete. A legacy heartbeat or Email Monitor dispatcher stage remains fallback-only until cutover and must not overlap the worker afterward.
