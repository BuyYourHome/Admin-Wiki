...
| Email Monitor | `email-monitor` | `019ecba7-f1cc-7ac1-aaf7-d89a3f21b582` | Route mailbox summaries, email intake routing, and Email Delivery mode to the WesStudio task. | Restored to the preserved WesStudio runtime after the 2026-08-24 rollback review; OfficeAssist task `01a029bf-81d2-76e1-9960-64558a57640b` remains inactive. |
...
| Quickbooks Invoice | `quickbooks-invoice` | `01a05967-9a05-7081-a62e-616b2d8e61fd` | Route only validated, authorized QuickBooks bill-creation handoffs from Invoice Entry after messaging and interim browser gates pass. | Dispatchable on `WES-VIDEOEDITOR` only for validated Invoice Entry handoffs. Preserve exact-company, duplicate-search, one-save, read-back, ambiguous-result stop, no-payment, no-send, and no-unrelated-bookkeeping gates. The prior `WESSTUDIO` task is superseded. |
| PR Messaging Dispatcher | `pr-messaging-dispatcher` | `01a05d0c-8031-7d92-9474-ab2330008ddb` on WES-VIDEOEDITOR | Infrastructure only; do not route business work here. | Five-minute heartbeat is active. Wakes exact local destination tasks from machine-addressed central records; unattended Quickbooks Invoice validation is pending. |
