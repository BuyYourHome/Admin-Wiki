...
| Quickbooks | `quickbooks` | `01a05967-9a05-7081-a62e-616b2d8e61fd` | Route by named mode. Current `Invoice` mode accepts only validated, authorized QuickBooks bill-creation handoffs from Invoice Entry after messaging and interim browser gates pass. | Pending renamed-identity validation on `WES-VIDEOEDITOR` - not dispatchable. Preserve exact-company, duplicate-search, one-save, read-back, ambiguous-result stop, no-payment, no-send, and no-unrelated-bookkeeping gates. The prior `WESSTUDIO` task is superseded. |
