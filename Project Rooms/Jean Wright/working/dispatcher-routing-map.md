...
| Quickbooks Invoice | `quickbooks-invoice` | `01a05967-9a05-7081-a62e-616b2d8e61fd` | Route only validated, authorized QuickBooks bill-creation handoffs from Invoice Entry after messaging and interim browser gates pass. | Pending messaging registration and authenticated Chrome readiness on `WES-VIDEOEDITOR` - not dispatchable. The prior `WESSTUDIO` task is superseded as the destination. |
