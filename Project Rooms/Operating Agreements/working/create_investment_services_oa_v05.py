...
OUT = OUT_DIR / "IS V05 - Investment Services OA Draft - from Reassembled OA Check.docx"
NOTES = OUT_DIR / "IS V05 - Investment Services OA Draft Notes.md"
...
def apply_version_footer(doc):
    for section in list(doc.sections)[:-1]:
        ...
        add_run(paragraph, " | IS V05 - Investment Services OA Draft")
...
def build():
    ...
    for paragraph in paragraphs:
        if paragraph.text.startswith("Current Manager(s). Joshua Kennedy"):
            replace_whole_paragraph(
                paragraph,
                "Current Manager(s). [UNCONFIRMED Manager(s)] will serve until replaced, resignation, or removal under this Agreement.",
            )
            break
    ...
    # Remove SYH-specific manager/signature blocks from the certification page for
    # Investment Services LLC's own OA.
        ...
        for row in table.rows:
            for cell in row.cells:
                ...
                for paragraph in list(cell.paragraphs):
                    text = paragraph.text.strip()
                    if text in {"Investment Services LLC, Manager", "Manager:"}:
                        remove_following = True
                    if remove_following and (
                        text == "Investment Services LLC, Manager"
                        or text == "Manager:"
                        or text == "Joshua Kennedy, Manager"
                        or text.startswith("By: ")
                        or text.startswith("Printed Name:")
                        or text.startswith("Title: Manager of Investment Services LLC")
                        or not text
                    ):
    ...
    review_markers = [
        ("Company as Pass-Through Entity.", "INVESTMENT SERVICES V05 TAX REVIEW: tax classification remains [UNCONFIRMED]; confirm before finalizing pass-through, partnership, or other tax language."),
    ]
    ...
    for paragraph in paragraphs:
        if paragraph.text.strip() == "CERTIFICATION":
            add_green_note_after(
                paragraph,
                "INVESTMENT SERVICES V05 CERTIFICATION NOTE: replace certification/signature blocks with Investment Services-specific Member(s), Manager(s), signer titles, and effective date after confirmation.",
            )
            break
    ...
    for paragraph in paragraphs:
        if paragraph.text.strip() == "EXHIBIT A":
            add_green_note_after(
                paragraph,
                "INVESTMENT SERVICES V05 EXHIBIT A NOTE: update Exhibit A to show Buy Your Home, LLC, Sell Your Home, LLC, Heritage Management LLC, and BYH 401K LLC as equal 25% Members, plus any confirmed contribution/unit details.",
            )
            break
    ...
    NOTES.write_text(
        "\n".join(
            [
                "# IS V05 - Investment Services OA Draft Notes",
                "",
                ...
                "Output DOCX:",
                "",
                "`IS V05 - Investment Services OA Draft - from Reassembled OA Check.docx`",
                "",
                "Wyoming OA use:",
                "",
                ...
                "- The Wyoming OA text says the agreement became effective when articles were filed; the visible file date is `25-01-17`, so V05 uses January 17, 2025 as the working effective date.",
                ...
                "- Replaces SYH-specific current-manager language for Joshua Kennedy with an Investment Services `[UNCONFIRMED Manager(s)]` placeholder.",
                ...
                "- Removes SYH-specific and Investment Services LLC-as-manager signature blocks from the certification page.",
                "- Adds review markers for unresolved tax, certification/signature, and Exhibit A issues.",
