...
OUT = OUT_DIR / "BYH OA Draft V5 Fresh Original-Based Redline - from Simplified OA.docx"
NOTES = OUT_DIR / "BYH OA Draft V5 Fresh Original-Based Redline Notes.md"
...
def main():
    ...
    insert_after(
        ...
        paragraphs[0],
        red_paragraph(
            "V5 FRESH ORIGINAL-BASED REDLINE: This draft restarts from the original Simplified OA source, not V4. It keeps the source's plural Manager(s) framework, identifies Browning Family Revocable Trust as the sole Member, gives LLC voting authority to the co-trustees Wesley Dale Browning and Jeanette Wilson Hollinger, and marks retirement-account provisions for removal where they do not apply.",
            paragraphs[0],
        ),
    )
    ...
    for p in list(body.findall("w:p", NS)):
        ...
        if (
            ...
            strike_paragraph(p)
            full_clause_notes.append((p, "BYH V5 NOTE: Retirement-account member/signature language does not apply. Use Browning Family Revocable Trust as the sole Member and co-trustee voting/signature authority."))
        elif txt.startswith("Note:") and "Account Holder" in txt and "IRA, HSA" in txt and "Coverdell ESA" in txt:
            strike_paragraph(p)
            full_clause_notes.append((p, "BYH V5 NOTE: This Section 3.7 Account Holder note does not apply to BYH and is struck through."))
            ...
            strike_paragraph(p)
            full_clause_notes.append((p, "BYH V5 NOTE: This retirement-account provision does not apply to BYH's stated trust-owned structure and should be replaced with ordinary LLC authority/conflict/tax language as appropriate."))
    ...
    for p in body.findall("w:p", NS):
        ...
        if txt == "Membership Units":
            section_notes.append((p, "BYH V5 MEMBER NOTE: Browning Family Revocable Trust is the sole Member holding 100% of the membership interest. The co-trustees are voting-authority holders for the Trust, not separate LLC Members."))
        elif txt.startswith("Voting Rights. Each Membership Unit gives the holder one vote."):
            section_notes.append((p, "BYH V5 VOTING NOTE: Voting authority for the sole Member, Browning Family Revocable Trust, is exercised by its co-trustees, currently Wesley Dale Browning and Jeanette Wilson Hollinger."))
        elif txt.startswith("Units held by an entity (like a business or trust) must be voted"):
            section_notes.append((p, "BYH V5 TRUST VOTING REPLACEMENT: Units held by Browning Family Revocable Trust are voted by its co-trustees, currently Wesley Dale Browning and Jeanette Wilson Hollinger, unless the Trust documents or Company records designate a different authorized trustee or representative."))
        elif txt == "Manager(s)" or txt.startswith("Manager(s) – Number and Tenure"):
            section_notes.append((p, "BYH V5 MANAGER NOTE: Keep the agreement flexible for one or more Manager(s). Wesley Dale Browning may be listed as an initial Manager, but the agreement should not be converted to single-manager-only wording."))
        elif txt == "CERTIFICATION":
            section_notes.append((p, "BYH V5 SIGNATURE NOTE: Signature blocks should show Browning Family Revocable Trust as sole Member, acting through co-trustees Wesley Dale Browning and Jeanette Wilson Hollinger, plus the Manager(s) signature block."))
        elif txt == "EXHIBIT A":
            section_notes.append((p, "BYH V5 EXHIBIT A NOTE: Exhibit A should list Browning Family Revocable Trust as the sole Member holding 100% of the membership interest. Co-trustees Wesley Dale Browning and Jeanette Wilson Hollinger should be shown as voting/signing authority for the Trust, not as separate Members."))
    ...
    for add in [
        red_paragraph("BYH V5 OPEN ITEMS: Confirm the full trust signature block, Manager(s) appointment language, S corporation election language, and whether any buy-sell provisions should survive in a sole-member trust-owned LLC."),
    ]:
        body.insert(idx, add)
        idx += 1
    ...
    NOTES.write_text(
        "# BYH OA Draft V5 Fresh Original-Based Redline Notes\n\n"
        f"Source used: `{SOURCE.name}`.\n\n"
        f"Output DOCX: `{OUT.name}`.\n\n"
        "Purpose:\n\n"
        "- Restart from the original Simplified OA source instead of layering on V4.\n"
        "- Preserve the original plural `Manager(s)` framework where appropriate.\n"
        "- Apply only the corrections now known for BYH.\n\n"
        ...
        "- The Section 3.7 Account Holder note does not apply and is struck through.\n"
        "- Retirement-account member/signature/prohibited-transaction/RMD language is marked for removal where it does not fit BYH.\n\n"
