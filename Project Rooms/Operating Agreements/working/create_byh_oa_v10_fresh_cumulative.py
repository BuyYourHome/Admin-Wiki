...
V9 = OUT_DIR / "BYH OA Draft V9 Article 2 Managers Tax Title Certification - from V8.docx"
...
def build_docx():
    ...
    # Article 2 cumulative structure.
    article2 = None
    old_21 = None
    for p in body.findall("w:p", NS):
        ...
        if txt == "Membership Units":
            article2 = p
        if txt.startswith("Membership Units Authorized."):
            old_21 = p
    ...
    if old_21 is not None:
        old = text_of(old_21)
        strike_paragraph(old_21)
        insert_after(body, old_21, make_red_para("2.2 " + old, old_21))
...
def write_notes_and_comparison():
    v10 = para_texts_from_docx(OUT)
    v9 = para_texts_from_docx(V9) if V9.exists() else []
    v10_full = "\n".join(v10)
    v9_full = "\n".join(v9)

    checks = [
        ("New Article 2.1", "2.1 Sole Member and Voting Authority." in v10_full, "2.1 Sole Member and Voting Authority." in v9_full),
        ("Former 2.1 preserved as 2.2", "2.2 Membership Units Authorized." in v10_full, "2.2 Membership Units Authorized." in v9_full),
        ("Current Manager(s)", "The current Manager(s) are Wesley Dale Browning, Jeanette Wilson Hollinger, and Investment Services LLC" in v10_full, "The current Manager(s) are Wesley Dale Browning, Jeanette Wilson Hollinger, and Investment Services LLC" in v9_full),
        ("5.1(b)(xi) retained", "Any increase or decrease in the number of Manager(s)." in v10_full, "Any increase or decrease in the number of Manager(s)." in v9_full),
        ("Tax heading conformed", "Company Tax Classification." in v10_full, "Company Tax Classification." in v9_full),
        ("Manager signature block", "MANAGER(S):" in v10_full and "Investment Services LLC, Manager" in v10_full, "MANAGER(S):" in v9_full and "Investment Services LLC, Manager" in v9_full),
        ("Exhibit A rebuilt as operative language", "Browning Family Revocable Trust\nc/o Wesley Dale Browning" in v10_full, "Browning Family Revocable Trust\nc/o Wesley Dale Browning" in v9_full),
    ]

    NOTES.write_text(
        "# BYH OA Draft V10 Fresh Cumulative Build Notes\n\n"
        ...
        "- Did not use V9 as the input document.\n"
        "- V9 was used only for comparison after generation.\n\n"
        ...
        "- Preserve 5.1(b)(xi).\n"
        "- Add sole-member language as new Article 2.1 and preserve former 2.1 as new 2.2.\n"
    ...
    rows = ["| Check | V10 Fresh Cumulative | V9 Layered |", "| --- | --- | --- |"]
    for label, v10_ok, v9_ok in checks:
        rows.append(f"| {label} | {'yes' if v10_ok else 'no'} | {'yes' if v9_ok else 'no'} |")
    COMPARE.write_text(
