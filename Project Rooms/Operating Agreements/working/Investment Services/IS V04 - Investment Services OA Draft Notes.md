...
- Structural DOCX checks passed for key Investment Services terms, member lines, omitted Article 5 entity-manager language, Section `5.1(b)(xi)` retention, Article 9 strikeouts, certification block cleanup, footer sections, and Drafting Legend presence.
- Direct LibreOffice PDF conversion passed and produced 26 pages.
- PDF text extraction confirmed the `IS V04 - Investment Services OA Draft` footer appears on agreement pages and does not appear on the final Drafting Legend page.
- Packaged DOCX-to-page-image rendering timed out in this environment, so page-image visual QA remains unavailable.
