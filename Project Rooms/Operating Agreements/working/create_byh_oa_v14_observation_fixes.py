...
OUT = OUT_DIR / "V14 - BYH OA Draft Observation Fixes - from Simplified OA.docx"
NOTES = OUT_DIR / "V14 - BYH OA Draft Observation Fixes Notes.md"
COMPARE = OUT_DIR / "V14 - BYH OA vs V13 Comparison.md"
V13 = OUT_DIR / "V13 - BYH OA Draft Clean Certification and Exhibit A - from Simplified OA.docx"
BYH_COLOR = "008000"
...
def build_docx():
    ...
    # Article 2 cumulative structure. Preserve source numbering and add stable 2.1(d).
    membership_description = None
    for p in body.findall("w:p", NS):
        txt = text_of(p).strip()
        if txt == "Description of the Membership Units.":
            membership_description = p
    if membership_description is not None:
        inserted = insert_lines_after(
            ...
            template=membership_description,
        )
    ...
    for p in body.findall("w:p", NS):
        ...
        elif txt == "Any increase or decrease in the number of Manager(s).":
            unstrike_paragraph(p)
    ...
    for p in body.findall("w:p", NS):
        ...
        if (
            ...
            strike_paragraph(p)
        elif txt.startswith("Borrowing Money.") and "However, Members described in Section 9.1" in txt:
            first, last = txt.split(" However, Members described in Section 9.1", 1)
            replace_paragraph_runs(
                p,
                [
                    ("normal", first + " "),
                    ("strike", "However, Members described in Section 9.1" + last),
                ],
            )
...
def write_notes_and_comparison():
    v14 = para_texts_from_docx(OUT)
    v13 = para_texts_from_docx(V13) if V13.exists() else []
    v14_full = "\n".join(v14)
    v13_full = "\n".join(v13)
    v14_exhibit = "\n".join(v14[v14.index("EXHIBIT A") :]) if "EXHIBIT A" in v14 else ""
    v13_exhibit = "\n".join(v13[v13.index("EXHIBIT A") :]) if "EXHIBIT A" in v13 else ""

    checks = [
        ("Original Membership Units Authorized paragraph present", "Membership Units Authorized." in v14_full, "Membership Units Authorized." in v13_full),
        ("Sole-member/trustee-voting language numbered as 2.1(d)", "2.1(d) Sole Member and Voting Authority." in v14_full, "2.1(d) Sole Member and Voting Authority." in v13_full),
        ("Opening ownership block line breaks", "This sole Member acts through its co-trustees," in v14_full and "Wesley Dale Browning and\nJeanette Wilson Hollinger, or" in v14_full, "This sole Member acts through its co-trustees," in v13_full),
        ("Opening agreement spacing artifact absent", "ofSell Your Home, LLCBuy Your Home, LLC" not in v14_full and "Operating Agreement of Sell Your Home, LLC Buy Your Home, LLC" in v14_full, "ofSell Your Home, LLCBuy Your Home, LLC" not in v13_full),
        ("Current Manager(s)", "The current Manager(s) are Wesley Dale Browning, Jeanette Wilson Hollinger, and Investment Services LLC" in v14_full, "The current Manager(s) are Wesley Dale Browning, Jeanette Wilson Hollinger, and Investment Services LLC" in v13_full),
        ("5.1(b)(xi) retained", "Any increase or decrease in the number of Manager(s)." in v14_full, "Any increase or decrease in the number of Manager(s)." in v13_full),
        ("9.5 Borrowing Money mostly retained", "Borrowing Money.  The Manager(s) may arrange for the Company to borrow money" in v14_full, "Borrowing Money.  The Manager(s) may arrange for the Company to borrow money" in v13_full),
        ("Certification date no longer stale", "effective June ____, 2025" not in v14_full and "effective ____________, 2026" in v14_full, "effective June ____, 2025" not in v13_full),
        ("Defined manager signature groups", "Wesley Dale Browning, Manager" in v14_full and "Jeanette Wilson Hollinger, Manager" in v14_full and "Investment Services LLC, Manager" in v14_full, "Wesley Dale Browning, Manager" in v13_full and "Investment Services LLC, Manager" in v13_full),
        ("Clean Exhibit A current member only", "Browning Family Revocable Trust\nc/o Wesley Dale Browning" in v14_exhibit and "Quest Trust Company" not in v14_exhibit and "Heritage IRA" not in v14_exhibit, "Browning Family Revocable Trust\nc/o Wesley Dale Browning" in v13_exhibit and "Quest Trust Company" not in v13_exhibit and "Heritage IRA" not in v13_exhibit),
    ]

    NOTES.write_text(
        "# V14 - BYH OA Draft Observation Fixes Notes\n\n"
        ...
        "- Did not use V13 as the input document.\n"
        "- V13 was used only for comparison after generation.\n"
        ...
        "- Preserve visible spacing around the opening agreement entity-name replacement.\n"
        "- Add the Article 2 sole-member/trustee-voting language as `2.1(d)`.\n"
        ...
        "- Preserve 5.1(b)(xi).\n"
        "- Preserve 9.5 Borrowing Money except for the last retirement-account/disqualified-person sentence.\n"
    ...
    rows = ["| Check | V14 | V13 |", "| --- | --- | --- |"]
    for label, v14_ok, v13_ok in checks:
        rows.append(f"| {label} | {'yes' if v14_ok else 'no'} | {'yes' if v13_ok else 'no'} |")
    COMPARE.write_text(
        "# V14 - BYH OA vs V13 Comparison\n\n"
        "V14 was generated fresh from the Simplified OA source using the cumulative instruction set. V13 was used only as a comparison point.\n\n"
        + "\n".join(rows)
        + "\n\n## Practical Difference\n\n"
        "- V14 changes the Article 2 sole-member/trustee-voting insertion to `2.1(d)`.\n"
        "- V14 preserves 9.5 Borrowing Money and strikes only the retirement-account/disqualified-person last sentence.\n"
        "- V14 keeps 5.1(b)(xi) operative and unstruck.\n"
        "- V14 strengthens the opening agreement spacing fix.\n",
        encoding="utf-8",
    )
