...
OUT = OUT_DIR / "IS V04 - Investment Services OA Draft - from Reassembled OA Check.docx"
NOTES = OUT_DIR / "IS V04 - Investment Services OA Draft Notes.md"
...
def apply_version_footer(doc):
    for section in list(doc.sections)[:-1]:
        ...
        add_run(paragraph, " | IS V04 - Investment Services OA Draft")
...
def build():
    ...
    # Remove Investment Services LLC as a manager/signature block from the certification
    # page for Investment Services LLC's own OA.
        ...
        for row in table.rows:
            for cell in row.cells:
                ...
                for paragraph in list(cell.paragraphs):
                    text = paragraph.text.strip()
                    if text == "Investment Services LLC, Manager":
    ...
    review_markers = [
        ("Company as Pass-Through Entity.", "INVESTMENT SERVICES V04 TAX REVIEW: tax classification remains [UNCONFIRMED]; confirm before finalizing pass-through, partnership, or other tax language."),
    ]
    ...
    for paragraph in paragraphs:
        if paragraph.text.strip() == "CERTIFICATION":
            add_green_note_after(
                paragraph,
                "INVESTMENT SERVICES V04 CERTIFICATION NOTE: replace certification/signature blocks with Investment Services-specific Member(s), Manager(s), signer titles, and effective date after confirmation.",
            )
            break
    ...
    for paragraph in paragraphs:
        if paragraph.text.strip() == "EXHIBIT A":
            add_green_note_after(
                paragraph,
                "INVESTMENT SERVICES V04 EXHIBIT A NOTE: update Exhibit A to show Buy Your Home, LLC, Sell Your Home, LLC, Heritage Management LLC, and BYH 401K LLC as equal 25% Members, plus any confirmed contribution/unit details.",
            )
            break
    ...
    NOTES.write_text(
        "\n".join(
            [
                "# IS V04 - Investment Services OA Draft Notes",
                "",
                ...
                "Output DOCX:",
                "",
                "`IS V04 - Investment Services OA Draft - from Reassembled OA Check.docx`",
                "",
                "Wyoming OA use:",
                "",
                ...
                "- The Wyoming OA text says the agreement became effective when articles were filed; the visible file date is `25-01-17`, so V04 uses January 17, 2025 as the working effective date.",
                ...
                "- Removes the Investment Services LLC manager/signature block from the certification page.",
