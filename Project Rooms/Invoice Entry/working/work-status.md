...
- Canonical skill and installed local `invoice-entry` skill were synchronized and recursive SHA-256 hash-verified across 10 files on 2026-08-14 after the opposite-signed Receipt posting rule was added. Receipt documents retain positive collected amounts; workbook entries use the negative equivalent through normal invoice placement.
...
| 18 | Receipt mode - 115 Rosebrooks Dr Estate Sale | Mode configured for `20-HM - 115 Rosebrooks Dr`. `Receipt item #<item number>` from Wes confirms sale and cash collection at the current established Marketplace price unless he states an exception. No specific sold-item command has been supplied yet. | On the first exact item command, resolve one Marketplace item/listing, generate one duplicate-checked Receipt as `Estate Sale Proceeds / Project Credit`, send one sold-status handoff to Marketplace, and post the negative equivalent through normal invoice placement. Use the supported Vendor Tab or `Review` with blank destination when category evidence is insufficient. |
| 19 | Tensity stove-sale receipt `RCPT-20260814-4121-001` | One `$250.00` cash receipt generated, QA-verified, and filed once to the Tensity `Owning/Invoices` folder. SharePoint read-back and synchronized-file SHA-256 verification passed. Buyer name, collector, and deposit are not recorded; no Marketplace item number was supplied. The new Receipt rule supports an `Appliances` workbook entry of `-$250.00`. | Do not repeat the receipt or filing. Insert once through the normal invoice workflow after a fresh workbook and duplicate check. If the stove corresponds to a Marketplace listing, Wes may supply its item number for a separate duplicate-safe sold-status handoff. |
| 20 | Rosebrooks cleanup-cash receipt `RCPT-20260814-115-001` | One `$300.00` cash receipt generated and QA-verified; retained in the Invoice Entry working archive. Property filing is held because `Property/20-HM-115 Rosebrooks Dr/Owning` has no established receipt destination. Payer, collector, and deposit are not recorded. The project is certain but no expense category is supported, so the new Receipt rule supports a `Review` entry of `-$300.00` with blank destination. | Do not recreate the receipt or invent a property filing folder. Insert once into `Review` through the normal invoice workflow after a fresh workbook and duplicate check; leave destination blank for category review. Do not record a deposit without separate evidence. |
