...
- Subject 3: `Re: Josh Kennedy time card invoice drafts - week ending 2026-07-24`
...
- Attachments: none
- Wes approval source: Outlook message `AQMkADZkMGMwNDI3LWU5ZjctNDgzMC1iMjdkLTY0NmRiN2IwNmIyZgBGAAADKGOvWJYjX0aEKaRpd0MYQQcAVSXargQY20aF7RLCkro4ggAAAgEMAAAAVSXargQY20aF7RLCkro4ggACf-9XCgAAAA==`, received `2026-07-29T13:56:16Z`. Wes replied `I approve` to the exact week-ending 2026-07-24 package.
...
These filed PDFs are historical approved artifacts. Under Wes's 2026-08-03 rule, their accepted time is consolidated into the single July 16-31 semimonthly invoice `INV-JKLLC-20260731-001`. They must not create payment obligations in addition to that invoice. The existing Tensity Review row requires reconciliation before any new posting.
...
The Wes-forwarded source received five seconds after Josh's July 22 message is duplicate transport evidence for the same 9-hour-5-minute line. It is preserved for traceability and was not added as a second Time Card line.

The Wes-forwarded source received five seconds after Josh's July 23 message is duplicate transport evidence for the same 10-hour-5-minute line. It is preserved for traceability and was not added as a second Time Card line.

The fresh Tensity workbook contained no invoice-number match for `TC-JK-20260724-TENSITY-001`. Invoice Entry added one Review row rather than guessing a Labor destination worksheet.
...
- Wes approved the exact worker/week/package on 2026-07-29. Approval does not indicate payment.
- Regenerated both invoices with `Worker verified / Approved by Wes` and `Approved by Wes - Ready for Payment`, then visually validated both pages.
- Filed the BackOffice invoice in `Office Admin/Invoices & Receipts`.
- Replaced the Tensity invoice in `Property/24-HM - 4121 Tensity Dr/Owning/Invoices` with the approved-status PDF.
- Resolved the live workbook to `Property/24_Project Management - 4121 Tensity Dr 2.xlsm`, found no matching invoice number, and added Review Row ID `IE-20260729-JOSH-TC-JK-20260724-TENSITY-001`.
- The Review row has blank `Destination Worksheet` and status `Needs Review` because Labor is not an approved Vendor Tabs destination. No vendor-tab row was guessed.
- Workbook validation passed after Excel save and reopen: `tblInvoiceReview` remained intact, the request defined name remained `=Review!$B$1`, formula and pre-existing error fingerprints were unchanged, and no external links were introduced.
- Email Monitor sent and verified the approved package to Josh on `2026-07-29T20:27:10Z`, with Wes and Jenny copied, using delivery request `IE-EMAIL-20260729-JOSH-TIMECARD-APPROVED-001`.
