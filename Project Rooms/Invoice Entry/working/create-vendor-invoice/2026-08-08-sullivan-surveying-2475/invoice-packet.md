# Sullivan Surveying Invoice 2475 Packet

## Status

- State: `Needs Wes - Vendor Math And Worksheet Placement`
- Project/property: `26-BYH - 908 Pond St` / `Lot 18 Burtrose Subdivision, 908 Pond Street`
- Vendor: `Sullivan Surveying, PLLC` (invoice header: `Sullivan Survey Company`)
- Invoice date: `2026-08-08`
- Invoice number: `2475`
- Invoice total as printed: `$387.00`
- Terms: `Accounts settled within 30 days of receipt`
- Payment status: `Not Approved / Not Paid - No Payment Authority`
- Duplicate status: `Two Outlook copies consolidated into one invoice obligation`

## Structured Fields

| Field | Value |
| --- | --- |
| `project_property` | `26-BYH - 908 Pond St - Lot 18 Burtrose Subdivision, 908 Pond Street` |
| `vendor_name` | `Sullivan Surveying, PLLC` |
| `invoice_date` | `2026-08-08` |
| `invoice_number` | `2475` |
| `invoice_amount` | `$387.00 as printed; math discrepancy held` |
| `work_category` | `Surveying / Town of Cary application and submittal fee - destination not approved` |
| `source_scan_path` | `Not applicable - routed Outlook attachment` |
| `saved_invoice_file_path` | `Not filed to a project folder; authoritative source retained in the Invoice Entry Working Archive` |
| `recommended_workbook` | `Property/26_Project Management - 908 Pond St 3.xlsm` |
| `recommended_worksheet` | `Needs Wes - no approved surveying/property-due-diligence worksheet` |
| `confidence_status` | `Needs Wes - Vendor Math And Worksheet Placement` |

## Authoritative Sources

- Dispatch id: `email-monitor-route-vendor-invoice-20260808-sullivan-2475-001`.
- Primary Outlook message id: `AQMkADZkMGMwNDI3LWU5ZjctNDgzMC1iMjdkLTY0NmRiN2IwNmIyZgBGAAADKGOvWJYjX0aEKaRpd0MYQQcAVSXargQY20aF7RLCkro4ggAAAgEMAAAAVSXargQY20aF7RLCkro4ggAChS2omgAAAA==`.
- Duplicate/corroborating Outlook message id: `AQMkADZkMGMwNDI3LWU5ZjctNDgzMC1iMjdkLTY0NmRiN2IwNmIyZgBGAAADKGOvWJYjX0aEKaRpd0MYQQcAVSXargQY20aF7RLCkro4ggAAAgEMAAAAVSXargQY20aF7RLCkro4ggAChS2omwAAAA==`.
- Both messages were received `2026-08-08T20:30:08Z`, had subject `FW: July Invoice`, and exposed the same attachment filename, MIME type, metadata size, and content ID.
- Original sender: `Daniel Smith <Daniel@sullivansurvey.com>`.
- Source attachment: `C:\Users\wesbr\Buy Your Home\Buy Your Home - Office Admin\Scanned Files\Invoice Entry Working Archive\Source Documents\2026-08-08 Sullivan Surveying 2475\invoice\Lot 18 Burtrose Subdivision (908 Pond Street) - Invoice No. 2475.pdf`.
- Retrieved file size: `107,785 bytes`; Outlook attachment metadata size: `108,311 bytes`.
- SHA-256: `C35B6EBCEE79CA9EAC304B6415C35FD9A8369A8CF4EECF0431B011F934A728E2`.

## Invoice Detail

The valid, unencrypted, one-page PDF bills Buy Your Home LLC for work associated with Lot 18 Burtrose Subdivision at 908 Pond Street:

| Date | Description | Quantity | Unit price | Printed line total |
| --- | --- | ---: | ---: | ---: |
| 2026-07-21 | Office work: fill out Town of Cary application, submit application, and upload map to portal | 1.25 | `$150.00` | `$187.00` |
| 2026-07-22 | Town of Cary submittal fee |  |  | `$200.00` |

The printed subtotal and total due are `$387.00`.

## Math Hold

The first line does not extend exactly: `1.25 x $150.00 = $187.50`, while the invoice prints `$187.00`. The printed lines total `$387.00`, but exact multiplication plus the `$200.00` fee would total `$387.50`. Invoice Entry preserved the vendor's printed `$387.00` obligation and did not alter, round, approve, or pay it.

## Routing And Duplicate Decision

- The exact project is supported by the attachment's project/address and the recognized Invoice Entry project list.
- Invoice `2475` is distinct from prior Sullivan Surveying invoice `2395` by invoice number, date, services, and amount.
- The two Outlook messages are duplicate transport copies of invoice `2475`, not separate obligations. One attachment copy was retrieved and retained.
- The readable attached invoice falls within direct Email Monitor/Create Vendor Invoice intake; raw scan/OCR extraction was not required from Doc Scan.
- Prior 908 Pond review established that surveying/property due diligence has no approved destination worksheet in `Property/26_Project Management - 908 Pond St 3.xlsm`.
- No workbook was opened or edited. No project-folder filing, invoice approval, payment, payment-link access, vendor contact, mailbox change, or paid-status action occurred.

## Decisions Needed From Wes

1. Confirm whether the vendor should issue a corrected invoice for the `$0.50` line-extension difference or whether the printed `$387.00` total should remain the review amount.
2. Choose an approved workbook destination for this and the earlier Sullivan Surveying invoice, such as an existing worksheet/category, Review placement, or another approved surveying/property-due-diligence destination.
