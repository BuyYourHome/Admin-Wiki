...
- A native in-cell checkbox can retain `CellControl.Type = xlTypeCheckbox`, its Boolean value, and correct selector logic while failing to draw in desktop Excel. If reapplying `SetCheckbox` does not restore visibility, add a visible Forms checkbox control over the approved checkbox cell and link it to the same Boolean cell. Validate the visible checked and unchecked states, the selector output, and the exact Teams-downloaded copy. Rose `Exterior!N2` required this repair on 2026-07-15.
