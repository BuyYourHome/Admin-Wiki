...
| `sources\2026-08-31-bathroom-wand-restore-correction-handoff-summary.md` | PR message source summary | authoritative | Bathroom Fixtures durable correction handoff asking Lowes Order to restore one uniquely missing wand-related selected component if the live cart comparison supported it. |
| `outputs\2026-08-31-bathroom-wand-restore-needs-wes.md` | Cart-correction output | authoritative result | Live cart comparison found multiple selected baseline lines missing, including both wand-related candidates, so no item was restored and the correction returned Needs Wes. |
| `sources\2026-09-01-angle-sander-sandpaper-request-summary.md` | User request summary | authoritative | Wes requested 120-grit sandpaper for pickup at the Maynard store with Josh as pickup person and provided the exact Lowe's product link. |
| `outputs\2026-09-01-angle-sander-sandpaper-cart-status.md` | Cart-fill output | authoritative result | Lowe's cart contains one Gator Mouse Refill 120 Grit 15pk, item `1348484`, model `3667`, for pickup at Morrisville Lowe's on Maynard Road. Pickup person was not set because the cart-review page did not expose that field before checkout. |
| Wes instruction to proceed through checkout for pickup-person and PO-number entry | User instruction | authoritative | Lowes Order may proceed through checkout only to enter or verify a confirmed pickup person and confirmed PO number, then must stop before payment, order submission, paid services, financing, protection plans, substitution approval, or final fulfillment scheduling that would commit the order. |
