...
|---|---|---|
| Should the new development chat also become the live automation `target_thread_id`? | This controls where future failure/blocker notices appear. Changing it affects live automation visibility. | Resolved: Wes approved creating a dedicated email-summary status thread instead. Current target thread id is `019ecba7-f1cc-7ac1-aaf7-d89a3f21b582`. |
...
- 2026-06-15: Live automation was converted from standalone cron to heartbeat attached to the dedicated `OfficeAssist Morning Email Summary Status` thread so daily runs stop creating new chats.
