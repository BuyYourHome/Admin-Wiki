...
- Maintain a stable current Jenny clickable chapter review packet at `outputs/jenny-chapter-review-current/Gracious Millionaire - Jenny Clickable Chapter Review - CURRENT.html`. After significant manuscript changes, regenerate this current packet from the latest full manuscript so Jenny can click chapter links and read the current chapter text one by one.
...
- When sending a clickable outline email, include the current manuscript length and a brief comparison to typical book lengths. Use the current manuscript word count when available, and compare it in plain language to common nonfiction ranges such as short book or novella length around 20,000-40,000 words, standard nonfiction around 50,000-80,000 words, and longer trade nonfiction above 80,000 words.
...
- For manual or non-heartbeat Gracious Millionaire work, after regenerating the current Jenny clickable chapter review packet for a significant manuscript change, send it from `OfficeAssist@BuyYourHomeLLC.com` to `Jenny@BuyYourHomeLLC.com` and copy `WesWill@BuyYourHomeLLC.com`, then verify the sent copy in OfficeAssist Sent Items.
- The project-room heartbeat still must not send email or take external actions. If a heartbeat creates a significant manuscript change, it should regenerate the current Jenny clickable chapter review packet, record the need to send it, and notify Wes that the email send is deferred because the heartbeat cannot send email.
