...
- For new substantive book/source material, use Interview mode first and then Rewrite mode. Interview mode should turn the new material into its own Jean-guided conversation or interview product with source boundaries. Rewrite mode should then independently rebuild the whole manuscript from the original project-room source material and current editorial rules so chapter context, themes, redundancy, and sequencing are reconsidered across the book.
