...
- New OfficeAssist instruction emails with a subject containing `gracious millionaire` should be routed into this room as source material by the OfficeAssist email monitor. The monitor should not attach a heartbeat to the Gracious Millionaire thread, create a new chat, or process the writing task unless Wes explicitly asks for processing there.
...
- Intake-heartbeat behavior is defined in [[intake-heartbeat-rules]]. The Gracious Millionaire heartbeat may act only on project-room files and logs dropped by OfficeAssist or other approved processes, not on mailboxes.

## Current Status

Started. Email source files have been created from the retrievable messages in the `WesWill@BuyYourHomeLLC.com` Outlook folder:
...
- Review the source files for completeness against Outlook.
...
- Create `working/officeassist-intake-log.md` before enabling the Gracious Millionaire intake heartbeat.
