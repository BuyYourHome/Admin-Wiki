# Document Scanning Automation Audit
...
| Dedicated status/development chat | Recent thread search for `Document Scanning` and `document-scanning` did not find a dedicated status chat. | Confirm whether a dedicated thread should be created or linked before changing automation config. |
