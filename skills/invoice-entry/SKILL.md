...
### Approved Invoice Completion Contract

- Wes's approval of a project-related invoice is standing authorization and an instruction to complete the supported non-payment downstream workflow. Without another `go`, Invoice Entry must finalize and file the approved invoice, insert every supported project allocation into its proper authoritative SharePoint project workbook, validate and read back each inserted row, and create any otherwise-ready QuickBooks handoff required by this skill.
- Approval satisfies the workbook-posting authority gate. Do not reinterpret project-spreadsheet insertion as a separate Wes authorization after the invoice is approved. Workbook mapping, duplicate risk, missing required data, unavailable authoritative files, or an unsafe save may still block an individual allocation, but each blocker must identify the exact invoice, project, workbook, worksheet or missing mapping, and required resolution.
- Use the exact invoice ID plus project/allocation identity as the stable duplicate key. Amount-only matches are not duplicate proof. Reopen or reread the authoritative SharePoint workbook after saving and verify the exact row values and destination.
- An approved project-related invoice is not operationally complete while any supported allocation is unposted. Do not return `Completed` merely because the PDF was generated, approved, filed, archived, emailed, or routed to QuickBooks. Return `Completed` only when every supported allocation is verified present and every other required non-payment destination has either completed or returned its own valid terminal result.
- If one or more allocations cannot be posted, preserve successful verified insertions and return `Blocked` or `Needs Wes` with an itemized allocation result. Never silently record `workbook_posted: false`, omit the allocation, or convert a posting blocker into a completed workflow.
- Approval does not authorize payment, a payment transaction, paid status, or unsupported accounting judgment. Those remain separately gated even when filing, spreadsheet insertion, and supported QuickBooks routing proceed.
- At every startup and backup-monitor run, reconcile approved project-related invoices in durable Invoice Entry state against their authoritative project workbooks. Automatically resume any supported missing insertion under the existing approval; do not ask Wes to repeat approval. Report an itemized blocker only when the insertion cannot be completed safely. This backstop applies to Tim Fleming, Josh Kennedy, and other approved project-related invoices.
...
- Every final Tim Fleming or Josh Kennedy Time Card invoice approved by Wes must be delivered through Email Monitor to both `WesWill@BuyYourHomeLLC.com` and `Jenny@BuyYourHomeLLC.com` with the exact approved PDF attached. A single Sent Items-verified approved-status delivery containing both recipients and the exact PDF satisfies this requirement; otherwise prepare one duplicate-safe supplemental package for the missing recipient. Jenny's receipt, review, or response does not constitute invoice approval, payment approval, or paid status; Wes remains the sole final approver.
...
1. Each accepted Time Card email or validated Manager packet updates the accumulated, source-traceable record for that worker's authorized billing period. Josh's periods are semimonthly: the 1st through the 15th and the 16th through the last calendar day. Tim Fleming is paid weekly; the current established Tim week runs Monday through Saturday. Do not put Tim on Josh's semimonthly cycle. The accumulated record is the source of truth; a generated PDF is a replaceable output, not the editable source.
2. Split actual time by project and BackOffice destination inside one invoice for the worker-specific period.
3. Generate one payable `INVOICE` for the complete worker-specific period. The invoice both requests payment and shows the project/BackOffice allocation; do not generate separate payable invoices per destination unless Wes expressly directs a supported exception.
...
6. After each meaningful time update, regenerate the draft invoice and route it through Email Monitor's Email Delivery workflow to the worker/payee, copying `WesWill@BuyYourHomeLLC.com` and `Jenny@BuyYourHomeLLC.com`, unless a worker-specific rule or Wes's exact recipient restriction says otherwise. Preserve that restriction exactly and do not add omitted recipients. The draft is the correction mechanism: show any reasonable working interpretation of correctable source wording, identify the uncertainty in the PDF and email, and state that no reply is needed when the time, project allocation, and totals are correct; the sender should reply only when a correction is needed. Do not hold a draft merely because a correctable time detail may change. A previously held correctable interval must not suppress later drafts: carry it under the most reasonable source-supported working allocation, label that interpretation in the PDF and delivery body, and reconcile any correction in place without creating another line. When a later correction changes any displayed time, project, allocation, description, or total, regenerate and redistribute the corrected draft to the authorized recipients, even if an earlier version was already delivered. The corrected delivery supersedes the earlier presentation for review but does not itself approve or finalize the invoice. If the worker/payee identity or delivery address is missing, prepare and preserve the source-supported draft facts but do not guess a recipient; request the missing contact before worker delivery. Tim Fleming is the worker-specific exception in rule 7 and must not receive Time Card emails.
7. Tim Fleming has a specific internal update-and-review rule. Incorporate every meaningful Tim time update into the current weekly accumulated record, regenerate the draft, and send that refreshed version through Email Monitor only to `WesWill@BuyYourHomeLLC.com` and `Jenny@BuyYourHomeLLC.com`, exactly once for that version. Tim must not appear in To, CC, or BCC on a Tim Time Card draft, approval request, correction, approved-invoice delivery, or status email. Do not wait for the pay period to close before sending an updated draft. Label an open-period request as accuracy review rather than payment approval. A response from Wes that contains no correction confirms the current draft version as factually correct. A response from Wes containing a correction authorizes applying that correction to the accumulated record and requires another refreshed draft to Wes and Jenny only. Jenny is included for visibility and recordkeeping; her response does not constitute accuracy confirmation or payment approval. An accuracy-review response never authorizes payment, filing, posting, finalization, or paid status.
8. An invoice cannot become final before its worker-specific billing period closes. Josh's semimonthly periods close on the 15th or last calendar day; Tim's current weekly period closes on Saturday. After the close, send the final Tim draft to Wes for explicit approval with Jenny copied. Only Wes may approve Tim's closed-period invoice for filing, posting, or payment eligibility. Do not infer that approval from an earlier accuracy-review response.
9. When Wes asks to see, review, or receive drafts for all Tim periods that are not approved, reconcile the complete accepted Tim period inventory before responding. Classify every period as `Approved`, `Closed - Awaiting Wes Approval`, or `Open - Not Yet Eligible For Approval`; exclude approved periods from the approval package and show the current open period separately for visibility. A request from Wes for those drafts authorizes delivery of the current closed-period drafts to Wes with Jenny copied, but it is not approval of any invoice and does not authorize payment, filing, posting, finalization, or paid status. Do not require another `go` after the inventory is reconciled.
10. For every closed Tim period without Wes approval, prepare the latest complete weekly draft and route it through Email Monitor to `WesWill@BuyYourHomeLLC.com` with `Jenny@BuyYourHomeLLC.com` copied, under the exact subject `Time Card Approval - Tim Fleming`. Tim must not receive or be copied on the approval request. A single approval-review email may contain multiple closed periods when each attachment and its period, hours, and amount are clearly identified. Do not include an open period in that approval email, do not resend an already delivered unchanged approval package, and do not omit a closed unapproved period merely because the current open-period draft also exists.
11. Before notifying Email Monitor, materialize the complete immutable Email Delivery package in the authoritative Teams operational packet folder. Include the unique delivery request ID, origin and callback identities, authorization basis, exact sender and recipients, subject, plain-text body, attachment paths and hashes, restrictions, and payload hash. Verify the saved Teams file by read-back, then hand Email Monitor both the canonical SharePoint identity and the current-machine resolved path; task-message text alone is not the delivery package. Invoice Entry retains ownership through the callback: monitor until OfficeAssist Sent Items verification or an exact blocker, resolve any safe recoverable pre-submission package or pointer defect, resume the same request without asking Wes to repeat authorization, and update Teams `work-status.md` with the terminal delivery result.

- When another Time Card email or validated Manager packet arrives for the same worker and billing period, reconcile it against the existing period record, add only new active lines, update explicit corrections in place, and retain the same invoice number.
...
- For both Tim Fleming and Josh Kennedy, invoice line descriptions must state only the supported work performed and relevant project context. Omit time-reporting and administrative commentary such as `no break taken`, `source`, references to source emails, repeated `hours` quantities, and `without start or end times`. Put dates, hours/quantities, rates, and amounts in their designated fields. Preserve original wording, breaks, source references, and missing-time details in internal supporting records; do not delete evidence or change calculations. If work detail is absent, do not invent a task. Keep any material unresolved interpretation visible in the appropriate review note or delivery body, outside the work description. Apply this to new and revised drafts and final invoices, and check the rendered description column before delivery. Description-only cleanup preserves invoice identity and approval when dates, accepted time, allocations, rates, and totals are unchanged; it does not authorize a duplicate email or new obligation.
...
- After final approval, route the exact approved Time Card invoice PDF to both Wes and Jenny under the approved-invoice delivery rule. Do not mark the Time Card delivery complete until Email Monitor returns Sent Items verification showing both required recipients, whether in one message or one reconciled supplemental delivery.
...
- Draft Time Card invoices use correction-by-exception review. Silence from the sender means no correction was reported; it is not a separate affirmative verification requirement. For Tim, only Wes's no-correction response affirmatively confirms that the current draft version is factually correct, and it is never payment approval; Jenny's response is visibility-only under the Tim-specific rule.
