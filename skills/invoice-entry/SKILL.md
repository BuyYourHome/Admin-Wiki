...
- creating and sending a free-text invoice verification request back to the proper vendor under Create Vendor Invoice when vendor identity and email address are clear,
...
- invoice approval, payment, accounting entries, vendor communication outside the Create Vendor Invoice free-text invoice verification request, or legal/financial decision-making.
...
- When sending Create Vendor Invoice emails with attachments through the Outlook connector, pass attachment paths as a list on the first attempt and verify the OfficeAssist Sent Items copy, including sender, recipients, copied recipients, subject, and attachment flag.
