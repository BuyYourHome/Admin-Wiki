...
- Put superseded Markdown manuscript and chapter-draft archive copies in the Teams `Archive` subfolder, not the Teams root. Name archive Markdown files with the zero-padded version first, using `v## - Title.md`, so Teams sorting groups historical drafts by version.
- Put superseded HTML review packets and ZIP packages in mode-specific or packet-specific subfolders under Teams `Archive`, such as `Archive\Rewrite Mode`, `Archive\Interview Mode`, `Archive\Dialogue Mode`, `Archive\Quick Mode`, `Archive\Jenny Review Packets`, and `Archive\Packages`.
