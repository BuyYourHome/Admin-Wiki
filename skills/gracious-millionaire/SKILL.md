...
- Put superseded Markdown manuscript and chapter-draft archive copies in the Teams `Archive` subfolder, not the Teams root. Name archive Markdown files with the zero-padded version first, using `v## - Title.md`, so Teams sorting groups historical drafts by version. Keep current HTML review packets and ZIP package copies in the Teams root unless Wes asks for a different folder.
