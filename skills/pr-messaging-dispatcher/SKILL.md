...
20. When Wes explicitly authorizes cancellation of an old queue-acknowledged Pending status record, use only `AdministrativeCancelAcknowledgedStatus`. Require the exact release `0.4.6` config, immutable hash and full-record version, Pending attempt, owner/generation/SID/machine, destination pin, adapter and CLI hashes, journal entry, permanent marker and exact notification, adapter result, queue UUID, absent receipt/result, and status-only no-business/no-production flags. Preserve the Pending attempt and evidence, mark delivery unresolved, claim neither delivery nor business completion, and reject late acceptance.
21. Treat immutable-hash compatibility as read-only. The canonical verifier may test a seven-digit UTC round-trip candidate only for typed `DateTime`/`DateTimeOffset` values or exact ISO UTC timestamp strings whose fractional zero suffix was shortened by JSON serialization; never rewrite the stored hash or payload and never normalize other strings.
...
- Install release `0.4.6` through `tools\pr-messaging\low-token\Install-LowTokenWorker.ps1`: `Stage`, one exact synthetic `StartValidation`, and `PromoteLive` only after verified completion. Existing live `0.4.0` or later machines use `UpgradeLive`, which preserves the owner generation, journal, state directory, task identity, schedule, and destination pins while replacing the hash-pinned package and task launcher. The guarded upgrade also reviews and pins the current normal-user Codex CLI path and hash; ordinary ticks never auto-trust a replacement CLI. Release `0.4.6` retains the closed-history and integrity-quarantine controls, bounded 180-second tick, and 60/120-second manager limits. A hash-invalid terminal record remains blocking unless Wes explicitly authorizes the exact machine-local owner to apply `AdministrativeQuarantineIntegrityFailure`; that action preserves the corrupted record and releases only its transport hold. `Rollback` preserves journals and central records.
