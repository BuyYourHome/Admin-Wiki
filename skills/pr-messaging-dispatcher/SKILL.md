...
- Install release `0.4.2` through `tools\pr-messaging\low-token\Install-LowTokenWorker.ps1`: `Stage`, one exact synthetic `StartValidation`, and `PromoteLive` only after verified completion. Existing live `0.4.0` or later machines use `UpgradeLive`, which preserves the owner generation, journal, state directory, task identity, schedule, and destination pins while replacing the hash-pinned package and task launcher. `Rollback` preserves journals and central records.
