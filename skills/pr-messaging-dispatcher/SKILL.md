...
21. Treat immutable-hash compatibility as read-only. The canonical verifier may test a seven-digit UTC round-trip candidate only for typed `DateTime` or `DateTimeOffset` values produced while reading an older Windows PowerShell record; never rewrite the stored hash or payload and never normalize arbitrary strings.
