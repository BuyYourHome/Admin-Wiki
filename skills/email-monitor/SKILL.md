...
The scheduled supervisor runs every 10 minutes independently on each assigned machine through `Invoke-CodexWorkflowHealthSupervisorHidden.vbs` using `wscript.exe`. Direct recurring `powershell.exe` execution is prohibited because it can display a console and interrupt the interactive user. `workflow-health-registry.json` keeps Invoice Entry enabled and Email Monitor disabled on `WESSTUDIO`. During migration, `officeassist-workflow-health-registry.json` keeps Email Monitor disabled until its correct-login task, restored runtime, connector access, and heartbeat are verified on `OFFICEASSIST`; its Doc Scan entry remains unchanged. Each supervisor evaluates only its machine's enabled entries, uses a machine-specific mutex, isolates malformed configurations, refuses the wrong machine, and does not depend on Outlook or another connector it supervises.
...
For Wes, `Time Cards\Tim` and `Time Cards\Josh` are final-approved-invoice archives. A message qualifies only when its controlling subject, body, or attachment establishes Wes's final approval of the invoice or transmits the exact final Wes-approved invoice. A factual-accuracy confirmation, correction response, open-period draft, daily or refreshed draft, approval request, or other review copy is not a final approved invoice.

Wes classification precedence:

1. `Time Cards\Tim`: a Tim Fleming message containing or transmitting the exact final Wes-approved invoice.
2. `Time Cards\Josh`: a Josh Kennedy or Josh Kennedy LLC message containing or transmitting the exact final Wes-approved invoice.
3. `Time Cards`: every other message whose controlling subject or content concerns Josh Kennedy's reported or worked hours or his Time Card invoice cycle, including source reports, corrections, clarifications, daily or refreshed drafts, approval requests, factual confirmations, and other time-related status. Also use `Time Cards` for a Tim Fleming message that reports Tim's hours worked or carries an open or unapproved Tim draft. Tim messages that do not concern hours or the invoice cycle continue through the remaining rules. These three Time Card classifications take precedence even when another folder label would otherwise match.
4. `Health and Failures`: failures, failed checks, health alerts, fraud alerts, or rollback instructions.
5. `Brynda Law Suit`: subjects identifying Brynda Suit or the associated mediation, possession, counterclaim, witness, settlement-position, or settlement-proposal work.
6. `Drafts for Review`: subjects identifying a draft, revised draft, or draft attachment.
7. `Approvals Needed`: subjects requesting approval, asking Wes to approve, or stating that Wes approval is needed.
8. `Daily Summaries`: Wes, Boss, Morning, OfficeAssist, Jenny, or Josh email/mailbox summaries.
9. `Manager Tasks`: subjects containing `Manager Task`, matched case-insensitively.
10. `Sent Confirmations`: approved-status notices, sent-and-verified notices, delivery verification, completed/run-complete notices, sender tests, or display-name verification.
11. `Invoice Entry`: invoices, cost-allocation reports, vendor verification, and related payment-report messages that do not match `Time Cards` and were not already classified as approval or confirmation.
12. `Project Rooms`: Gracious Millionaire, GM Site, manuscript, Codex/dispatcher/computer setup, document-scan, closing-document, MOU, insurance-report, or other identifiable Project Room work.
13. `Other`: anything that does not match a class above.
...
For Jenny, `Time Cards\Tim` and `Time Cards\Josh` use the same final-approved-invoice standard as Wes's folders. Jenny's review, visibility, or factual response does not establish Wes's final approval.
...
5. `Brynda Law Suit`: subjects identifying Brynda Suit or the associated mediation, possession, counterclaim, witness, settlement-position, or settlement-proposal work.
6. `Drafts for Review`: subjects identifying a draft, revised draft, or draft attachment.
7. `Approvals Needed`: subjects requesting approval, asking Jenny to approve or review, or stating that Jenny action is needed.
8. `Daily Summaries`: Jenny, Josh, Wes, Boss, Morning, OfficeAssist, or other email/mailbox summaries delivered to Jenny.
9. `Manager Tasks`: subjects containing `Manager Task`, matched case-insensitively.
10. `Sent Confirmations`: approved-status notices, sent-and-verified notices, delivery verification, completed/run-complete notices, sender tests, or display-name verification.
11. `Invoice Entry`: invoices, cost-allocation reports, vendor verification, and related payment-report messages that do not match `Time Cards` and were not already classified as approval or confirmation.
12. `Project Rooms`: Gracious Millionaire, GM Site, manuscript, Codex/dispatcher/computer setup, document-scan, closing-document, MOU, insurance-report, or other identifiable Project Room work.
...
- classify messages directly inside the activated mailbox's `Inbox/Jean Wright`; additionally inspect messages directly inside its `Time Cards` child folder and move only qualifying final Wes-approved invoices into `Time Cards\Tim` or `Time Cards\Josh` so an existing backlog is corrected;
- leave daily drafts, refreshed drafts, source reports, correction threads, factual confirmations, approval requests, and all other non-final Time Card messages in the parent `Time Cards` folder; do not reclassify messages already inside `Time Cards\Tim`, `Time Cards\Josh`, or any other child folder;
