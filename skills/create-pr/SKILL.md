...
5. Verify one dedicated `PR Messaging Dispatcher - <COMPUTERNAME>` task and validated release `0.4.0` deterministic worker on the target computer. Require an unattended cross-machine validation and machine-scoped exclusive ownership before live promotion; do not leave the legacy heartbeat or Email Monitor dispatcher stage active afterward.
