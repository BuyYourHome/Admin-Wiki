...
- Current element map: `C:\Codex\Wiki Files\Project Rooms\REI BlackBook\working\gracious-millionaire-element-map-047.md`
...
- Homepage direction: build a coherent sequence consisting of a book-centered opening, a Wes and Jenny introduction, central book themes, an approved featured passage or quotation when available, an inside-the-book/progress section without publishing unapproved manuscript text, a clear update-interest form, and a complete footer.
- Run duration: when a fresh lock is acquired and useful backlog remains, spend approximately 45-90 minutes on one coherent design objective, or continue until that objective is implemented and QA'd. The 30-minute schedule is a wake-up cadence, not a limit on active work; the overlap lock causes later invocations to yield while a run is active.
- Default action is inspect current state, select one coherent design objective from the current map and iteration ledger, implement it across every affected page or shared component, QA desktop and mobile where practical, refresh the map, and record the result.
- Do not end after one tiny copy or image change when additional changes are required to complete the selected objective. A successful iteration should produce a visible, cohesive improvement or a newly verified blocker with a different useful objective completed in the same run.
- Treat the full public site as the redesign scope. Work through homepage, shared header/navigation, The Book/About content, author story, journal/blog/category surfaces, update/contact forms, thank-you pages, sidebars, footer, responsive behavior, links, image placement, and remaining template assets until each is intentionally designed or intentionally removed.
- Before browser work, read the latest element map and `gm-mode-iteration-ledger.md`. Resume the highest-priority unfinished objective unless current public state makes another dependency more urgent. Do not restart the audit from zero on every run.
...
- When REI BlackBook rules authorize a completion-backlog email after a substantive GM run, prepare only the final delivery package and hand it to the existing Email Monitor status task `01a03956-fe55-7f62-9c0a-17c18f763320` requesting Email Delivery mode. Do not send directly from this Project Room. Do not mark the email sent until Email Monitor returns verified Sent Items evidence with the sent message id and timestamp.
