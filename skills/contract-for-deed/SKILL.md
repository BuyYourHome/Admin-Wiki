...
## Installed Skill And Development
...
9. Prepare the delivery package to `WesWill@BuyYourHomeLLC.com` only so Wes can review and forward it himself.
...
Do not hardcode package-file sharing URLs in the CFD email-body builder. For each external-access Email Package run, create or refresh anonymous view links for the actual current files in the Teams `Contract Package` folder, write those current links to `Contract Package\Email Package\current-sharepoint-view-links.json`, and build the body with link-manifest enforcement. Validate that every displayed email link comes from the current manifest and that no stale hardcoded URL remains before sending.

When the Outlook connector only supports plain-text message bodies, do not request a plain-text send for a linked/polished Email Package unless Wes explicitly accepts plain text for that run. The Contract for Deed handoff must identify that the package requires an HTML-capable send path, provide the HTML and plain-text fallback body paths, attach the complete package ZIP, and state the Wes-only recipient restriction. Email Monitor's Email Delivery mode owns choosing the connector or local Outlook fallback, sender safety, and Sent Items verification.

CFD must track two email delivery package types:

- **Plain-text package:** use when the email body does not need hidden/display hyperlinks or polished HTML formatting. Contract for Deed prepares the package and hands it to Email Monitor's Email Delivery mode for OfficeAssist delivery and verification.
- **HTML cover-sheet package:** use when sending the CFD cover-sheet / closing-package email with clean filename links. Contract for Deed prepares the HTML body, plain-text fallback, displayed Teams/SharePoint links, and complete package ZIP, then hands the package to Email Monitor's Email Delivery mode with the requirement for an HTML-capable send path. Any sender exception, including `WesWill@BuyYourHomeLLC.com`, must be explicitly authorized by Wes for that run and included in the handoff.

For the Email Monitor handoff, pass the package ZIP path as an absolute attachment path. Do not pass newline-separated paths.
...
Do not copy scratch files, extracted text, rendered QA images, logs, staged spreadsheets, prototypes, or temporary working files to Teams as package deliverables. Do not use Teams copies as the source for regeneration unless Wes identifies a Teams-edited file as the version to preserve.

For repository cleanup only, move generated render previews, LibreOffice profiles, Python caches, benchmark/run-metric batches, temporary workbook backups, and superseded working artifacts to the separate Teams `Codex Working Archive` recorded in `C:\Codex\Wiki Files\Project Rooms\Contract for Deed\working\teams-working-archive-map.md` after they are processed and verified. Do not place cleanup artifacts in the active Teams `Contract Package`.
...
For package documents generated by another workflow, do not rewrite their substantive content or footers from CFD. The authoring workflow owns its document version unless Wes creates a specific exception. The Amortization Chart PDF is authored by the Amortization workflow, so CFD should receive and package the returned PDF and version without supplying or rewriting the Amortization version. Credit Worthiness reports and affidavit/support documents are CWE-authored, so CFD should not rewrite them; CFD should package the versions supplied by CWE.
