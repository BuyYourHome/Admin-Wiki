...
6. When Wes starts a new OA build, asks for a refresher, asks for the build stack, or asks where the OA build process stands, send the OA build-order refresher from `working\oa-version-shortcuts.md`: Jeff Base OA, Wes Simplified Draft, Simplified OA, Simplified OA Accepted Build Copy, Segmented OA, Reassembled OA Check, Mode Rules / Inputs, Entity Drafts, Approved Final OAs. Include the linked Mode Rules / Inputs and Entity Drafts examples shown in that shortcut file, and state whether any Approved Final OAs exist.
