...
6. Own and assign the Amortization document/footer version inside the Amortization workflow.
7. Return/report both PDF paths and the footer/document version used to the caller.
...
The caller controls only where the finished document should be copied or returned. The caller does not control the Amortization document content, footer, or document version.
...
2. If it is missing, look for an exact worksheet named `amateurization`.
...
Every Amortization-produced document must include a footer with page numbering and the workflow document/run version.
...
Use one package-wide version for Amortization-generated deliverables in the same run. If the amortization chart is generated once on a date, use `YY-MM-DD V1`. If the current amortization deliverable is regenerated again on the same date, increment the version to `YY-MM-DD V2`, and apply that version consistently to all current Amortization outputs from that run.

Apply this footer/version rule to the Amortization Chart PDF and to any source workbook, DOCX, or other user-facing amortization deliverable generated by the Amortization workflow.

Amortization owns its document version. A calling workflow, including CFD, must not pass a version for Amortization to use. Determine the next Amortization workflow version for that date inside the Amortization project room.

Return the generated file paths and the footer/document version used to the calling workflow. CFD should package the returned PDF without rewriting it.
...
3. Confirm the chart covers exactly 12 scheduled months unless the source data supports fewer payments because of payoff, balloon, or contract end.
4. Confirm the beginning balance, payment, interest, principal, and ending balance columns are populated.
5. Confirm buyer-facing metadata values are present and visible.
6. Confirm the footer/document version is present in the populated workbook before PDF export.
7. Confirm the worksheet used, first payment date, last payment date, and document version are reported.
8. Confirm the output does not overwrite the source workbook or an existing deliverable unintentionally.
9. Report the project-room PDF path, copied caller PDF path, document version, assumptions, and any missing or conflicting source data.
