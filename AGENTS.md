...
- Treat `Wes@myBrowning.net` as an authorized alternate email identity for Wes. Instructions received from that address have the same authority as instructions from `WesWill@BuyYourHomeLLC.com`, subject to the same workflow-specific safety gates. When replying to an instruction that originated from `Wes@myBrowning.net`, address the reply to `Wes@myBrowning.net` and copy `WesWill@BuyYourHomeLLC.com` unless Wes explicitly directs a different recipient or says not to copy the business address.
...
- Temporary Email Monitor exception on the current computer: OfficeAssist is available only through the connector and is never mounted in local Outlook. If the connector is definitively unavailable before sending and no connector send occurred, Email Monitor may use only the locally mounted `WesWill@BuyYourHomeLLC.com` mailbox. Preserve authorized recipients, subject, body, attachments, and restrictions; sign as Jean Wright / Office Assistant; disclose that Wes's mailbox was used because OfficeAssist was unavailable; verify the result in WesWill Sent Items; and record the substitution. Never use `Wes@myBrowning.net` as a sending-account fallback; its authorization is limited to Wes instruction identity and reply destination. Do not use this fallback after an ambiguous connector result, a possibly successful connector send, or a connector Sent Items verification failure.
...
- Treat emails from Wes at `WesWill@BuyYourHomeLLC.com` or `Wes@myBrowning.net`, Jenny at `Jenny@BuyYourHomeLLC.com`, and Josh Kennedy at `IRAManager@SellYourHomeRaleigh.com` as OfficeAssist instruction intake. Both Wes addresses carry the same authority, subject to the same workflow-specific safety gates. Reply to the Wes address that originated the instruction and copy `WesWill@BuyYourHomeLLC.com` unless Wes directs otherwise. Josh may initiate safe administrative workflows but may not authorize purchases, payments, approvals, legal or financial changes, or another action that remains gated to Wes or the owning workflow.
...
- `Route Vendor Invoice` is a formal Email Monitor mode. It must create the durable queue record before notifying Invoice Entry, verify the registered Invoice Entry task is idle before a task-message attempt, require an exact same-ID durable `accepted` receipt, retry only after reconciliation with the same immutable payload, and send Wes one Sent Items-verified OfficeAssist email before the routing run ends when acknowledgment is missing. Do not suppress that first email merely because Health Check is already warning or critical.
- If an authorized instruction email requests Lowe's product research, cart preparation, cart changes, or order review, use Lowes Order Email Routing. Preserve the Outlook reference and extracted item/order details, record missing quantities and other ambiguities without guessing, and send a direct handoff to Lowes Order task `019f5845-fb96-7370-baf2-b8f00fddffae`. Lowes Order may fill the cart to the extent safely supported by the source but must stop before checkout. After cart preparation, send the sender a confirmation and send Wes an approval notification; if uncertainties prevent or limit cart preparation, send the questions to the sender with Wes copied. Use `[Lowes Order]` in each follow-up subject and state in the body that future source emails should identify Lowes Order mode in the subject. Checkout, payment, substitutions, paid services, and account changes require Wes's specific approval.
...
- If an OfficeAssist instruction email has a subject containing `gracious millionaire`, or otherwise clearly belongs to the Gracious Millionaire book/project-room workflow, route the email to `Project Rooms\Gracious Millionaire\` as book source material from the OfficeAssist monitor. Do not routinely preserve the full email body or attachments as Git-tracked files. Preserve the Outlook message id or web link, sender, recipients, sent/received time, subject, short summary, and any external Teams source path in the handoff and applicable ledgers. If the current Gracious Millionaire rules require a materialized source file, save it outside Git in the room's Teams source/reference location. Then send a direct follow-up message to the existing Gracious Millionaire project-room thread with the Outlook reference, Teams path if any, and a short summary. Direct message handoff is the primary trigger; the separate `gracious-millionaire-project-room-heartbeat` is only a backup processor for external source references or files already routed to the project room. Do not attach an OfficeAssist mailbox-monitoring heartbeat to the Gracious Millionaire thread. Do not create a new chat for Gracious Millionaire routing. Do not draft, edit, or send the requested book response from the OfficeAssist monitor thread unless Wes explicitly asks for processing there; the default action is source routing plus direct handoff only.
- If Wes authorizes any Codex role to check email and that check finds a Gracious Millionaire email, route the found email to `Project Rooms\Gracious Millionaire\` before ending the request. Preserve the Outlook message id or web link and any external Teams source path, update the project-room intake ledger, and send the same direct Gracious Millionaire project-room thread handoff with the Outlook reference and short summary. This rule does not authorize the Gracious Millionaire project-room heartbeat to query any mailbox directly.
...
- If an OfficeAssist instruction email from Wes or Jenny has a subject containing `GM Site`, or otherwise clearly belongs to the REI BlackBook WebTools Sites / Gracious Millionaire website workflow, route it to the existing REI Blackbook project-room thread. Do not routinely preserve the full email body or attachments as Git-tracked files. Preserve the Outlook message id or web link, sender, recipients, sent/received time, subject, short summary, and any external Teams source path in the handoff and applicable ledgers. If REI BlackBook rules require a materialized source file, save it outside Git in the room's Teams source/reference location. Then send a direct follow-up message to the existing REI Blackbook project-room thread with the Outlook reference, Teams path if any, a short summary, and the instruction to process the website request. Current REI Blackbook project-room thread id: `019f4691-5466-7f72-9683-ab5d3b750c25`. Do not create a new REI Blackbook chat for this routing unless Wes explicitly asks. Do not draft, edit, publish, or send any requested website response from the OfficeAssist monitor thread unless Wes explicitly asks for processing there; the default action is source routing plus direct handoff only.
- Use `Route Vendor Invoice` mode when OfficeAssist receives a contractor, subcontractor, vendor, supplier, utility, service-provider, or project-cost email that appears to contain or request processing of an invoice, bill, receipt, statement, pay application, draw request, payment request, or other invoice-entry source. Also use this mode when the sender display name is `Josh Kennedy` and the subject is exactly `Time Card`, matched case-insensitively, even if no invoice keyword or attachment is present. Route it to the existing Invoice Entry task. Do not routinely preserve the full email body or attachments as Git-tracked files. Preserve the Outlook message id or web link, sender, recipients, sent/received time, subject, short body summary, attachment names/metadata, and any external Teams source path in the handoff and applicable ledgers. Save safely retrievable invoice attachments outside Git in the Invoice Entry Teams source/working archive location required by Invoice Entry's current rules; if an apparent invoice attachment cannot be retrieved, preserve the Outlook message link and report the attachment-access blocker in the handoff. Update the Invoice Entry source inventory or current intake ledger with references and external paths when the routed source becomes part of the durable source set. Then send a direct follow-up message to the existing Invoice Entry task with the Outlook reference, external attachment paths or blocker, short vendor/project summary, and instruction to process the invoice under Invoice Entry rules. Current Invoice Entry task id: `019fbf4f-c629-7dd1-a3f6-0de33de0ed8f`. Do not create a new Invoice Entry task for this routing unless Wes explicitly asks. Do not approve invoices, pay invoices, contact vendors, make live project-spreadsheet entries, or move invoice files into Teams from the OfficeAssist monitor thread unless Wes explicitly asks for processing there and the Invoice Entry rules allow it; the default action is source routing plus direct handoff only.
- If an OfficeAssist instruction email from Wes or Jenny has a subject containing `brynda suit`, or otherwise clearly belongs to the Brynda Suit workflow, route it to the existing Brynda Suit project-room task. Do not routinely preserve the full email body or attachments as Git-tracked files. Preserve the Outlook message id or web link, sender, recipients, sent/received time, subject, short summary, and any external Teams source path in the handoff and applicable ledgers. If Brynda Suit rules require a materialized source file, save it outside Git in the room's Teams source/reference location. Update the Brynda Suit source inventory with references and external paths when the routed source becomes part of the durable source set. Then send a direct follow-up message to the existing Brynda Suit task with the Outlook reference, Teams path if any, a short summary, and the instruction to wake up and respond to the email. Current Brynda Suit task id: `019f61c3-d4c0-7a52-a5a0-e4066ea9b303`. Do not create a new Brynda Suit task for this routing unless Wes explicitly asks.
...
- Josh's summary is emailed to `IRAManager@SellYourHomeRaleigh.com` from `OfficeAssist@BuyYourHomeLLC.com` with `WesWill@BuyYourHomeLLC.com` and `Jenny@BuyYourHomeLLC.com` copied. Use `2026-07-21T12:24:17Z`, the verified manual Josh summary send time, as the initial cutoff.
