...
- If an OfficeAssist instruction email from Wes or Jenny has a subject containing `GM Site`, or otherwise clearly belongs to the REI BlackBook WebTools Sites / Gracious Millionaire website workflow, route it to the existing REI Blackbook project-room thread. Preserve the email as its own Markdown source file under `Project Rooms\REI BlackBook\sources\email\`, including sender, recipients, sent/received time, subject, message id or web link when available, and body text. Update the REI BlackBook source inventory when the routed source becomes part of the durable source set. Then send a direct follow-up message to the existing REI Blackbook project-room thread with the routed source path, a short summary, and the instruction to process the website request. Current REI Blackbook project-room thread id: `019f4691-5466-7f72-9683-ab5d3b750c25`. Do not create a new REI Blackbook chat for this routing unless Wes explicitly asks. Do not draft, edit, publish, or send any requested website response from the OfficeAssist monitor thread unless Wes explicitly asks for processing there; the default action is source routing plus direct handoff only.
- Use `Route Vendor Invoice` mode when OfficeAssist receives a contractor, subcontractor, vendor, supplier, utility, service-provider, or project-cost email that appears to contain or request processing of an invoice, bill, receipt, statement, pay application, draw request, payment request, or other invoice-entry source. Also use this mode when the sender display name is `Josh Kennedy` and the subject is exactly `Time Card`, matched case-insensitively, even if no invoice keyword or attachment is present. Route it to the existing Invoice Entry task. Preserve the email as its own Markdown source file under `Project Rooms\Invoice Entry\sources\email\`, including sender, recipients, sent/received time, subject, message id or web link when available, body text, and available attachment names/metadata. Save invoice attachments beside the routed email source when the connector or local mailbox path can safely retrieve them; if an apparent invoice attachment cannot be retrieved, preserve the Outlook message link and report the attachment-access blocker in the handoff. Update the Invoice Entry source inventory or current intake ledger when the routed source becomes part of the durable source set. Then send a direct follow-up message to the existing Invoice Entry task with the routed source path, attachment paths or blocker, short vendor/project summary, and instruction to process the invoice under Invoice Entry rules. Current Invoice Entry task id: `019f3d56-b310-75c0-b084-616bfc1e9f59`. Do not create a new Invoice Entry task for this routing unless Wes explicitly asks. Do not approve invoices, pay invoices, contact vendors, make live project-spreadsheet entries, or move invoice files into Teams from the OfficeAssist monitor thread unless Wes explicitly asks for processing there and the Invoice Entry rules allow it; the default action is source routing plus direct handoff only.
