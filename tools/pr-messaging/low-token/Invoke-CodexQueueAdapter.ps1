...
if($LiveConfigPath){
    ...
    if($cfg.release -notin @('0.3.0-assisted','0.4.0') -or $DispatcherTaskId -cne $cfg.dispatcher_task_id -or $PayloadHash -cnotmatch '^[0-9a-f]{64}$' -or $CliPath -cne $cfg.cli_path -or $ExpectedCliHash -ine $cfg.cli_sha256){throw 'LiveAdapterIdentityMismatch'}
    ...
    if($cfg.release -ceq '0.4.0' -and !(Test-LtPinnedDestination $cfg $r.destination)){throw 'LiveDestinationNotPinned'}
    $a=@($r.attempts)[-1]
