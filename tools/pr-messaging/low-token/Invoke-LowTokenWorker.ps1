...
function Get-SafeAdapterErrorCode([string]$Text) {
    foreach($code in @('CliExecutableMissing','CliReleaseMismatch','LiveAdapterIdentityMismatch','LiveAdapterWindowsIdentityMismatch','LiveDestinationNotPinned','LiveAdapterClaimMismatch','LiveSubmissionAlreadyMarked','ReleaseChangedBeforeSubmission')){
        if($Text -match ('(?<![A-Za-z0-9])'+[regex]::Escape($code)+'(?![A-Za-z0-9])')){return $code}
    }
    $null
}
...
try {
    ...
    foreach($r in @($scoped|Sort-Object created_at_utc,message_id)){
        ...
        if(Test-LtProvenPreSubmissionFailure $answer $productionMarker){
            ...
            if($errorCode){$detail+=' Safe error code: '+$errorCode+'.'}
            ...
            $entry.submission_evidence=@{timed_out=$answer.timed_out;exit_code=$answer.exit_code;stdout_sha256=(Get-LtSha256 $answer.stdout);stderr_sha256=(Get-LtSha256 $answer.stderr);error_code=$errorCode;accepted=$false;queue_acknowledged=$false;queue_message_id=$null;submission_marker_present=$false}
            Save-Journal;$health.reconciled+=@{message_id=$r.message_id;outcome=$reconciled.outcome;reason='ProvenPreSubmissionFailure';error_code=$errorCode}
            break
        }
