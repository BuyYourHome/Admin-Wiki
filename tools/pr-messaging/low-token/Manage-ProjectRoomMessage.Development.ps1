...
. "$PSScriptRoot\Manager.Extensions.ps1"
# Staged copy: deliberately incapable of operating on the real share.
Assert-LtFixture $FixtureRoot @($QueuePath,$ClientConfigPath,$ManifestDirectory)
if ($ForceOffline -or $Action -in @("SyncSpool","Initialize")) { throw "UnsupportedDevelopmentOperation" }
...
Assert-MessageId -Id $MessageId
if ($Action -in @("Inspect","ConditionalClaim","ReconcileAttempt")) { Invoke-WithQueueLock { Invoke-LtManagerOperation | ConvertTo-Json -Depth 30 }; return }
Invoke-WithQueueLock {
