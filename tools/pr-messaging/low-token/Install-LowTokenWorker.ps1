...
$dependencies=@(@{path='..\Message-Integrity.ps1';sha256=(Get-FileHash -LiteralPath (Join-Path $PSScriptRoot '..\Message-Integrity.ps1')).Hash;placement='Inside the same version-pinned package root, never shared across releases'})
[ordered]@{
    ...
    destination=(Join-Path $env:LOCALAPPDATA 'BuyYourHome\PRMessaging\low-token\releases\0.2.0\low-token')
    planned_schedule='Every 60 seconds, continuously, interactive normal Windows user; no SYSTEM/password task'
    default_mode='Shadow';live_enabled=$false;files=$files;dependencies=$dependencies
