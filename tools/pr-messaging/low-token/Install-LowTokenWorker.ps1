...
[ordered]@{
    schema_version=1;release='0.1.0';status='PlanOnly';machine=$env:COMPUTERNAME
    ...
    destination=(Join-Path $env:LOCALAPPDATA 'BuyYourHome\PRMessaging\low-token\releases\0.1.0')
    ...
    prerequisites=@('Review this development release and failure results','Independently approved fresh worker canary and exclusive test ownership','Read-only desktop busy/pending probe validated','Reboot/login/offline/upgrade proof','Reviewed manager deployment with legacy ownership guard; drain before switching','Verify pinned package and client/manifest/ownership generation before activation')
