...
* `ReturnsTable[Total Profit]` should sum the cash-flow columns from `Initial Capitalization` through `Year 10`.
* `ReturnsTable` should not include a `Net Profit` column unless specifically requested.
* `ReturnsTable` should separate `Capital Investment` from `Basis / Property Payment`.
* For Sponsor, `SponsorCapitalInvestment` is the actual sponsor cash/cost invested and should be separate from `SponsorBasis` / `SponsorPropertyPayment`.
* In `ReturnsTable`, use `Initial Capitalization` instead of a visible `Year 0` column so the first operating year is shown as `Year 1`.
* In `ReturnsTable`, keep the ending columns after `Year 10` in this order: `Total Profit`, `Total Distributions`, `Equity Multiple`, `ROI %`, `IRR`.
* `Total Profit` should equal the sum of Initial Capitalization through Year 10 cash flows; `Total Distributions` should equal Total Profit plus Capital Investment.
...
* Do not add an intermediate Waterfall column for cash remaining after Class A priority payments. Calculate Class B capital return, deferred preferred return payment, and residual distributions directly from cash available less Class A capital return, Class B capital return, and Class A preferred return paid.
...
* Waterfall preferred-return balance should be an end-of-year balance:
  * `A Pref Balance` should equal prior-year `A Pref Balance` plus current-year `Pref Accrued` less current-year `Pref Paid`.
  * `Pref Paid` should use prior-year `A Pref Balance` plus current-year `Pref Accrued` as the available preferred-return obligation, not current-year `A Pref Balance`.
* Keep the Class A preferred-return columns ordered as `Pref Accrued`, `Pref Paid`, `A Pref Balance`.
* Do not reintroduce a separate `A End Accrued Pref` column unless specifically requested.
...
* The sponsor property payment is an Initial Capitalization Sponsor distribution and should not be treated as Class A capital return, Class B capital return, or Sponsor residual.
* Waterfall should keep Sponsor property payment in a separate Sponsor column from Sponsor residual.
* Current waterfall priority is capital-first deferred preferred return: Class A capital return, then Class B capital return, then accrued Class A preferred return, then residual split.
...
* Sponsor residual share should be editable in the workbook; current value is 50%.
...
* Any model year greater than `SaleExitYear` should show zero activity in the Annual Model, Waterfall, and Returns Table.
* The sale year itself may show final operations, sale proceeds, payoff, and waterfall distributions; only later years should be zeroed out.
* `SaleExitYear` is selected from option buttons beside the Waterfall table.
* Do not place an exit-year option button on the internal Year 0 / Initial Capitalization row.
* The Waterfall refinance option-button helper cell is `Waterfall!R3`, named `SelectedWaterfallRefiOption`.
* The Waterfall exit option-button helper cell is `Waterfall!S3`, named `SelectedWaterfallExitOption`.
* Waterfall exit-year buttons should be shown only for Years 1-10, with no visible button labels.
* `Assumptions!B5` should equal the selected helper value, constrained to Years 1-10.
...
* Display the Rent Scenario Schedule and use the active scenario based on the effective-year schedule.
...
## PowerPoint Template And Layout Rules

* When using a prior PowerPoint as a template, reuse the existing slide layout, placeholders, object positions, and visual geometry where practical.
* Do not treat a PowerPoint template as only a color/font source. The alignment, box sizes, spacing, image areas, and master/layout structure are part of the template.
* Prefer editing existing text boxes in place over deleting template objects and adding new text boxes with approximate coordinates.
* If a slide must be rebuilt, match the prior deck's layout geometry before changing content.
* Keep metric values, labels, cards, and text frames centered within their visual boxes.
* Do not leave old text layers behind new text layers. Inspect slides for duplicate or overlapping text after edits.
* Fit images proportionally inside their intended picture areas. Preserve aspect ratio; do not stretch PNGs, charts, or concept images to fill a box.
* Keep text clear of images and charts. Leave visible spacing between captions, explanatory text, and media.
* For chart screenshots or exported workbook graphics, size the image to fit inside a clean content area and preserve aspect ratio.
* Avoid relying only on slide count and text extraction as verification. PowerPoint updates require visual checks for overlap, readability, contrast, image scaling, and alignment.
* If the user has manually moved a chart, table, picture, or text block, preserve that placement unless the user explicitly asks to move it.
* When asked to make formatting-only PowerPoint changes, do not move charts, tables, pictures, or major layout objects unless movement is specifically requested.
* Keep STR operating scenarios toward the back of investor decks when the base co-living / medium-term case is intended to remain the control underwriting case.
* Always copy or move generated project assets into the project folder. Do not leave project images only in the Codex generated-images cache.
