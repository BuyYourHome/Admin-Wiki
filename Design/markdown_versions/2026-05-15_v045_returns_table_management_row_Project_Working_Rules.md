...
* `ReturnsTable[Total Return]` should sum the annual cash-flow columns from Year 0 through Year 10.
* `ReturnsTable` should not include a `Net Profit` column unless specifically requested.
* In `ReturnsTable`, keep `Total Distributions`, `Equity Multiple`, and `ROI %` after `Year 10` and before `Total Return`.
* `ReturnsTable` should include a `Management` row that shows the annual management fee from `Operating Expense Budget!B15` / named range `AnnualManagementFee`.
