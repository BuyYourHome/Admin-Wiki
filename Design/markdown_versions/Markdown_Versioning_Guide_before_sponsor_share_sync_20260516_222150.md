...
Updated the waterfall so residual distributions after capital return priorities are shared proportionally among Class A, Class B, and Sponsor. At that time, formula-based residual shares were Class A 33.33%, Class B 50.00%, and Sponsor 16.67%, based on Class A capital of 300,000, Class B capital of 450,000, and Sponsor basis of 150,000.
...
After the Cost Analysis `A6:C11` summary section was removed, repaired the broken funding-gap reference and added a specific sponsor property payment to the sponsor-controlled bridge. At that time, the sponsor property payment was 150,000, the selected sponsor-controlled low case increased to 900,000, Class B completion capital increased to 600,000, and the payment was shown as a Year 0 Sponsor distribution in the Waterfall.
...
Added a `Total Return` column to `ReturnsTable` on the Waterfall tab. The column sums Year 0 through Year 10 cash-flow columns for each participant.

### 2026-05-15 - v042

Split insurance into two separate planning figures. The Cost Analysis sponsor-controlled bridge now itemizes construction-period insurance at 10,000 low / 25,000 high, while the Operating Expense Budget now labels completed property insurance separately at 4,900 per year.

### 2026-05-15 - v043

Added a rent scenario schedule to the Assumptions tab. Premium suite rents are now modeled at 1,350 low, 1,600 medium, and 1,750 high. Co-living suite rents are now modeled at 850 low, 1,100 medium, and 1,250 high. The Annual Model uses the low rent case through the stabilization/refinance year, the medium rent case in Year 3, and the high rent case from Year 4 forward.

### 2026-05-15 - v044

Added editable effective-year controls for each rent scenario on the Assumptions tab. The current schedule starts low rent in Year 1, medium rent in Year 2, and high rent in Year 4. The Annual Model now selects the rent case with the latest effective year that is less than or equal to the model year, so the low rent scenario ends after Year 1.

### 2026-05-15 - v045

Updated `ReturnsTable` on the Waterfall tab. Removed the `Net Profit` column, moved `Total Distributions`, `Equity Multiple`, and `ROI %` after `Year 10` and before `Total Return`, and added a `Management` row tied to the annual management fee from the Operating Expense Budget.

### 2026-05-15 - v046

Reoriented the Assumptions tab rent scenario schedule so rents increase left to right from low to medium to high. Changed stabilized monthly gross rent to the average of the low, medium, and high monthly gross rent cases. Updated management income in `ReturnsTable` to calculate dynamically as 6.00% of each year's effective gross income.

### 2026-05-15 - v047

Renamed the Returns Table `Total Return` column to `Total Profit` and reordered the ending return metrics to `Total Profit`, `Total Distributions`, `Equity Multiple`, and `ROI %`.

### 2026-05-15 - v048

Simplified the Waterfall capital tracking by removing `A End Capital` and `B End Capital`. The remaining `A Capital Balance` and `B Capital Balance` columns now carry forward unpaid capital directly from the prior year's balance less prior-year capital returned.

### 2026-05-15 - v049

Simplified the Waterfall preferred-return tracking by removing `A End Accrued Pref`. The remaining `A Pref Balance` column now carries forward unpaid Class A preferred return directly from the prior year's preferred balance plus prior-year preferred return accrued less prior-year preferred return paid.

### 2026-05-15 - v050

Reordered the Class A preferred-return columns in `WaterfallTable` to `Pref Accrued`, `Pref Paid`, and `A Pref Balance`.

### 2026-05-15 - v051

Changed `A Pref Balance` in `WaterfallTable` to represent the end-of-year unpaid preferred-return balance. `Pref Paid` now uses prior-year `A Pref Balance` plus current-year `Pref Accrued`, while `A Pref Balance` equals prior-year balance plus current-year accrued preferred return less current-year preferred return paid.

### 2026-05-15 - v052

Updated the Annual Model, Waterfall, and Returns Table so any year greater than `SaleExitYear` shows zero activity. Verified with `SaleExitYear` set to 4 that model years 5-10 zero out.

### 2026-05-15 - v053

Moved `IRR` to the last column of `ReturnsTable`. The ending Returns Table columns are now `Total Profit`, `Total Distributions`, `Equity Multiple`, `ROI %`, and `IRR`.

### 2026-05-15 - v054

Separated Sponsor capital investment from Sponsor basis/property payment. Added a `Sponsor Capital Investment` assumption and updated `ReturnsTable` to show separate `Capital Investment` and `Basis / Property Payment` columns. Sponsor ROI and equity multiple now use Sponsor capital investment rather than property basis.

### 2026-05-15 - v055

Renamed the visible `Year 0` column in `ReturnsTable` to `Initial Capitalization`. The Returns Table now presents Initial Capitalization followed by Year 1 through Year 10, while retaining the underlying timing needed for IRR calculations.

### 2026-05-16 - v056

Added Waterfall exit-year option buttons. This was later superseded by separate refinance and exit selectors: refinance now links to `Waterfall!R3`, named `SelectedWaterfallRefiOption`, and exit now links to `Waterfall!S3`, named `SelectedWaterfallExitOption`.

### 2026-05-16 - v057

Rebuilt the Waterfall exit-year option buttons so they appear only beside Years 1-10, with no option button on the internal Year 0 / Initial Capitalization row. Removed visible option-button labels and changed `Assumptions!B5` so the selected helper value maps directly to the selected sale/exit year.

## Current Waterfall Status

Updated the current model status to a Capital Recovery First / Deferred Preferred Return waterfall. Current priority is Class A capital return, then Class B capital return, then accrued Class A preferred return, then residual split. Current residual shares are Sponsor 40.00%, Class A 20.00%, and Class B 40.00%. Current refinance and exit controls use separate option-button helper cells: `Waterfall!R3` for refinance year and `Waterfall!S3` for exit year.
