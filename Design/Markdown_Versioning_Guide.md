...
Updated the current model status to a Capital Recovery First / Deferred Preferred Return waterfall. Current priority is Class A capital return, then Class B capital return, then accrued Class A preferred return, then residual split. Current residual shares are Sponsor 50%, Class A 18%, and Class B 32%. Current refinance and exit controls use separate option-button helper cells: `Waterfall!R3` for refinance year and `Waterfall!S3` for exit year.
